...
│  主仓 + be + M9 wrapper 都 import;npm workspace 软链;71 测试 │
└──────────────────────────────────────────────────────────────┘
└───────────────────────────────┬──────────────────────────────┘
...
7. **post-MVP / 已知边界**:DNS rebinding 扩展侧无 DNS 关不掉,需 ADR-0006 strict proxy;scoring v2 叠加模型需新增 cap 字段;`ui/`、`api/` 文档子层待补(README 步骤 3/4)。
