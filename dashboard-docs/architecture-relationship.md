...
- **iframe 内嵌录制已正式否决**(见 `system/adapter-recorder-system/adr/0008-reject-iframe-embedded-recording.md`):录制靠扩展 CDP `debugger.attach({tabId})` 抓真实 top-level tab,iframe 没有 tabId、目标站 `frame-ancestors`/`X-Frame-Options` 禁嵌、且 dashboard 自身 `default-src 'self'` CSP 先拦跨源 src。录制目标页维持「扩展拥有的真实 tab」;真要「当前页内操作」唯一可行的是 CDP screencast(`FEATURE_DIRECT_CDP_CAPTURE` 预留,未实现)。
...
| **Post-M10 · LLM 合成 adapter** | 截图采集接进录制链路 + Anthropic API 综合「截图+API痕迹」生成 adapter func/columns | 把 init 模板的 `func: return []` / `columns: []` 留白(本就是 TODO)从「人手填」升级成「LLM 生成 + 人工审阅」 | ✅ **MVP 落地(feature-gated,默认关)**:① **截图**——`handleCaptureRead` 结束录制时 best-effort 抓 `Page.captureScreenshot`(jpeg)存进 session sample(只驻内存不落盘,失败不阻断录制);② **LLM 合成**——be `src/llm/synthesize.ts`(注入式 Anthropic client,`claude-opus-4-8`,structured-output 约束 `{funcBody,columns,description,access}`;无 key/失败→null 退回空模板)+ config 加 `RECORDER_LLM_API_KEY`/`RECORDER_LLM_BASE_URL`/`FEATURE_LLM_SYNTHESIS`/`RECORDER_LLM_MODEL`(项目命名空间,避撞 Claude Code 的 ANTHROPIC_*);③ **模板插值**——recorder-core `renderAdapterTemplate` 加可选 `funcBody`(原样插入,刻意打破 no-raw-user-code)/`columns`/`description`/`access`(JSON 转义),**缺省字节级不变**;provenance 标 `@generated-by adapter-recorder-llm` + `@model`;daemon `/v1/init` 透传 + 回 `generatedSource`;④ **handleInit** dry-run 调合成、结果按 candidateId 缓存在 session(write 复用同一份→审阅的代码===写盘的代码、不重复调 LLM);⑤ **前端 InitStep** dry-run 展示 `generatedSource` + AI 生成/数据外发 banner,过 ADR-0005 审阅才写盘。**安全**:LLM 生成 func 靠 ① verify-runner 子进程隔离 ② dry-run 人工审阅 ③ provenance 标注三层兜底;截图+真实请求/响应外发 Anthropic API 已写进 ADR-0005 egress 条款。**测试**:recorder-core init 22 / be synthesize 5 + init-llm 2 + 截图 2 / Tier A 加「LLM on→真 daemon 渲染→真 adapter 文件含 func→真 verify-runner 执行得 rows>0」全绿。**第二阶段(设计已定·未实现)**:UI 节点录制(用户真实 click/input 序列)+ 多模态时间线喂 LLM——权威设计见 `system/adapter-recorder-system/12-llm-synthesis-and-ui-recording.md`(数据模型=复用 `src/observation/events.ts` 多轨 trace + 因果对齐;UI 捕获=扩展 CDP `Runtime.addBinding`+`bindingCalled` 复刻 network-capture buffer+轮询;input 值脱敏)。 |
| **M10** MVP 验收 | 11 章 MVP Acceptance 全项通过 | 收口 | ✅ **验收通过(2026-06-25)**:13 行验收表逐行映射到通过的测试/证据;三套测试全绿(recorder-core 84 / dashboard-be 73 / 主仓 4834 + 1 skipped);schema gate 本地校验通过(3 OpenAPI 3.1.0/无 nullable、bundle `$defs`、ref 全指向 bundle)。四留尾全清(#5 flag 接线 / #1a–d daemon·runner 观测 / #2 nearest-adapter / #4 fd3 forge = post-MVP)。真 daemon e2e〔Tier A,ubuntu-only〕每个 PR 由 CI(ci.yml `recorder-e2e`)强制;**覆盖率 ≥80% 当前未接 gate**(无 @vitest/coverage-v8、ci.yml 无阈值)= 待补 follow-up,本地仅以 7 核心模块 84 测试为证 |
| **Post-M10 · 候选聚拢 + 双轨评分 + prompt 压缩** | 同 endpoint 按 method+host+pathname 聚拢、参数取并集、LLM 双轨(rule delta + semantic bonus)评分;之上做 prompt 体积压缩 | be `src/llm/{score,generate,synthesize,responseSummary}.ts`;权威 14/15 号文档 | ✅ **落地未提交(2026-07-03)**:①**评分重构**(14 号)commit 680b6aab——双轨求分、候选聚拢、article_rank 30/low→88/high。②**prompt 压缩**(15 号)——score:paramObservations TOON(省54%,真机A/B无损)+ PROMPT_A 精简 4388→3916 + evidence(selector 两侧砍/urlParams 只留证明键/count:0 过滤)+ 数据段 minify(-38%);generate:选中过滤(85KB→12KB,修 prompt 混入全部候选 bug)+ minify(-45%)+ paramUnion/recommendedColumns TOON(多候选线性省,JSON-cell 转义防 LLM 中文含逗号);前端展示层去重 score + generate 按选中动态预览(codex 复审修 useCallback/seq guard 竞态)。be 292 测试全绿。 |

## Post-M10 · 一体化录制 + 录制体验(2026-06-29,未提交)

> 这批在 MVP 之后,围绕「在 dashboard 内完成录制」与「生成脚本质量/体验」。详细设计见模块 12 各 `M-*` 段 + adr/0003·0008 修订;实现进度见 memory `dashboard-inline-recording-screencast` / `recorder-scoring-and-seed-gap` / `recorder-pipeline-ux-and-daemon-concurrency`。

- **混合录制模式**(录制画面进 dashboard):录制形态提升为**应用层策略**(bind 时选),两种并存——
  - `tab_projection`(投屏,默认,通用):目标页在扩展真 tab,dashboard 截图轮询投屏其像素 + CDP `Input.*` 回传操作。不受 X-Frame-Options。
  - `embedded_iframe`(页内嵌入,仅不反嵌公开站,`FEATURE_EMBEDDED_IFRAME_RECORDING` gate 默认关):dashboard 自己的 tab 嵌跨源目标 iframe,扩展 attach **dashboard tab**,经 OOPIF flat autoAttach 录 iframe 内请求(带 frameSessionId)。CSP `frame-src` 三态(flag off 无 / on 无 override→`https:` / on+`RECORDER_IFRAME_FRAME_SRC`→指定 origin,只放 https)。噪音过滤放**采集适配层**(扩展 read 函数按目标 frameSessionId+descendants 过滤,丢顶层 dashboard 噪音)。adr/0008 由「全盘否决」修订为「混合·墙①被 OOPIF 打穿、墙②反嵌仍是投屏的理由」。
- **dashboard seed 输入**(评分质量):录制时 A/B 各填搜索关键词→be 用它解析 value→param→`deriveEvidenceSeedArgs` 喂 rank,恢复 `seed_arg_maps_to_param`(+20)/`response_echoes_seed`(+10)两信号(否则候选分上限 30、普遍 15 low)。raw seed 只在 be 内存用一瞬、不落盘(adr/0003 修订,守 M7c)。
- **pipeline UX 三改**:① 多选保存——`/recorder/save` 兼容批量 `{drafts:[...]}`、一次 `ranked→done`、回 `saved[]` 列表(前端勾选 + 结果列表);② verify 受限并发——`runPipeline` 串行 verify 改两阶段并发(上限 4)+ 轮询 1000→300ms(**架构事实:daemon RunnerPort 本就内置并发队列,串行是浪费**);③ 投屏滚动顺滑——LivePreview 交互期升频 180ms + wheel rAF 合并 delta。
- **adapter 落盘**:保存的脚本 = `~/.bycli/clis/<site>/<command>.js`(site=选中候选接口 host 的 slug、command=pathname 末段 slug;`dashboard/src/models/adapterName.ts`),报告 `~/.bycli/sites/<site>/recorder/<command>-report.json`。
