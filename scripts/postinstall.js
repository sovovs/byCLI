...
const BASH_COMPLETION = `# Bash completion for bycli (auto-installed)
_bycli_completions() {
...
const ZSH_COMPLETION = `#compdef bycli
# Zsh completion for bycli (auto-installed)
_bycli() {
  local -a completions
...
const FISH_COMPLETION = `# Fish completion for bycli (auto-installed)
...
function main() {
  ...
  try {
    switch (shell) {
      case 'zsh': {
        ...
        const completionFile = join(completionsDir, '_bycli');
      ...
      case 'bash': {
        ...
        const completionFile = join(userCompDir, 'bycli');
      ...
      case 'fish': {
        ...
        const completionFile = join(completionsDir, 'bycli.fish');
        ensureDir(completionsDir);
  ...
  const bycliDir = join(home, '.bycli');
  const spotifyEnvFile = join(bycliDir, 'spotify.env');
  ensureDir(bycliDir);
  if (!existsSync(spotifyEnvFile)) {
    ...
    console.log(`  Edit the file and add your Client ID and Secret, then run: bycli spotify auth`);
  }
