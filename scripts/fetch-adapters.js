...
/**
 * Sparse adapter sync: keeps ~/.bycli/clis/ clean by removing stale overrides.
 *
 ...
 *
 * ~/.bycli/clis/ is a sparse override layer, not a full copy.
 * Only eject-ed or user-modified sites appear here.
 *
 * Only runs on global install (npm install -g) or explicit BYCLI_FETCH=1.
...
const BYCLI_DIR = join(homedir(), '.bycli');
const USER_CLIS_DIR = join(BYCLI_DIR, 'clis');
const MANIFEST_PATH = join(BYCLI_DIR, 'adapter-manifest.json');
const PACKAGE_ROOT = resolve(import.meta.dirname, '..');
const BUILTIN_CLIS = join(PACKAGE_ROOT, 'clis');

function log(msg) {
  console.log(`[bycli] ${msg}`);
}
...
export function fetchAdapters() {
  ...
  // Skip if already installed at the same version (unless forced via BYCLI_FETCH=1)
  const isForced = process.env.BYCLI_FETCH === '1';
  ...
  //    Only eject-ed sites live in ~/.bycli/clis/.
  let cleared = 0;
  ...
  // 4. Clean up legacy compat shim files from ~/.bycli/
  // These were created by an older approach that placed re-export shims directly
  // in ~/.bycli/ (e.g., registry.js, errors.js, browser/). The current approach
  // uses a node_modules/@sovovs/bycli symlink instead.
  ...
  let tmpCleaned = 0;
  try {
    for (const entry of readdirSync(BYCLI_DIR)) {
      if (entry.startsWith('.plugins.lock.json.tmp-')) {
        try {
          unlinkSync(join(BYCLI_DIR, entry));
          tmpCleaned++;
        } catch { /* ignore */ }
      }
    }
...
function main() {
  ...
  const isExplicit = process.env.BYCLI_FETCH === '1';
  const isFirstRun = process.env._BYCLI_FIRST_RUN === '1';
