...
describe('browser tab CLI e2e', () => {
  ...
  it('lists, creates, and closes tabs through the built CLI', async () => {
    ...
    daemons.push(daemon);
    const env = { BYCLI_DAEMON_PORT: String(daemon.port) };
  ...
  it('keeps untargeted browser commands on the default tab after creating a new tab', async () => {
    const daemon = await startFakeDaemon();
    daemons.push(daemon);
    const cacheDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bycli-browser-tabs-'));
    cacheDirs.push(cacheDir);
    const env = {
      BYCLI_DAEMON_PORT: String(daemon.port),
      BYCLI_CACHE_DIR: cacheDir,
    };
