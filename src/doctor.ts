/**
 * bycli doctor — diagnose browser connectivity.
 *
...
export async function runBrowserDoctor(opts: DoctorOptions = {}): Promise<DoctorReport> {
  ...
  if (daemonFlaky) {
    ...
    issues.push('Daemon is not running. It should start automatically when you run an bycli browser command.');
  }
  ...
  if (extensionFlaky) {
    ...
    if (health.state === 'profile-required') {
      issues.push(
        ...
        '  Run bycli profile list, then bycli profile use <name>, or pass --profile <name>.',
      );
    } else if (health.state === 'profile-disconnected') {
      issues.push(
        `Selected browser profile is not connected: ${health.status?.contextId ?? 'unknown'}.\n` +
        '  Open that Chrome profile and make sure the byCLI extension is enabled.',
      );
    } else {
      issues.push(
        'Daemon is running but the Chrome/Chromium extension is not connected.\n' +
        'If the extension is already installed, try: bycli daemon restart\n' +
        'If the extension is not installed:\n' +
        '  1. Download from https://github.com/sovovs/byCLI/releases\n' +
  ...
  if (extensionConnected && !extensionVersion) {
    issues.push(
      ...
      '  Reload or reinstall the extension from: https://github.com/sovovs/byCLI/releases',
    );
  }
  if (!connectivity.ok) {
  ...
  if (extensionVersion && opts.cliVersion && extensionCompatRange) {
    if (!satisfiesRange(opts.cliVersion, extensionCompatRange)) {
      issues.push(
        ...
        '  Update the CLI: npm install -g @sovovs/bycli\n' +
        '  Or download a compatible extension from: https://github.com/sovovs/byCLI/releases',
      );
    }
    ...
    if (extMajor !== cliMajor) {
      issues.push(
        ...
        '  Download the latest extension from: https://github.com/sovovs/byCLI/releases',
      );
    }
  }
  ...
  if (extensionVersion && latestExtensionVersion && isNewerVersion(latestExtensionVersion, extensionVersion)) {
    issues.push(
      ...
      '  Download from: https://github.com/sovovs/byCLI/releases',
    );
  }
...
export function renderBrowserDoctorReport(report: DoctorReport): string {
  const lines = [`bycli v${report.cliVersion ?? 'unknown'} doctor` + ` (${getRuntimeLabel()})`, ''];
