...
const { mockExecFileSync, mockPlatform, mockSpawnSync } = vi.hoisted(() => ({
...
describe('executeExternalCli', () => {
  ...
  it('does not spawn a binary that is still unavailable after auto-install', () => {
    const docker: ExternalCliConfig = {
      name: 'docker',
      binary: 'docker',
      install: { mac: 'brew install --cask docker' },
    };
    mockExecFileSync
      .mockImplementationOnce(() => {
      ...
      .mockReturnValueOnce(Buffer.from(''))
      .mockImplementationOnce(() => {
        const err = new Error('docker still not found') as NodeJS.ErrnoException;
        err.code = 'ENOENT';
        throw err;
      });

    executeExternalCli('docker', [], [docker]);

    expect(mockExecFileSync).toHaveBeenNthCalledWith(1, 'which', ['docker'], { stdio: 'ignore' });
    expect(mockExecFileSync).toHaveBeenNthCalledWith(2, 'brew', ['install', '--cask', 'docker'], { stdio: 'inherit' });
    expect(mockExecFileSync).toHaveBeenNthCalledWith(3, 'which', ['docker'], { stdio: 'ignore' });
