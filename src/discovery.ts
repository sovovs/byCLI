 ...
 * user CLIs in ~/.bycli/clis/ can `import { cli } from '@sovovs/bycli/registry'`.
 *
 * This is the sole resolution mechanism — adapters use package exports
 * (e.g. `@sovovs/bycli/registry`, `@sovovs/bycli/errors`) and
 * Node.js resolves them through this symlink.
 */
export async function ensureUserCliCompatShims(baseDir: string = USER_BYCLI_DIR): Promise<void> {
  await fs.promises.mkdir(baseDir, { recursive: true });

  // package.json for ESM resolution in ~/.bycli/
  const pkgJsonPath = path.join(baseDir, 'package.json');
  const pkgJsonContent = `${JSON.stringify({ name: 'bycli-user-runtime', private: true, type: 'module' }, null, 2)}\n`;
  try {
  ...
  // Create node_modules/@sovovs/bycli symlink pointing to the installed package root.
  const bycliRoot = PACKAGE_ROOT;
  const symlinkDir = path.join(baseDir, 'node_modules', '@sovovs');
  const symlinkPath = path.join(symlinkDir, 'bycli');
  try {
    let needsUpdate = true;
    try {
      const existing = await fs.promises.readlink(symlinkPath);
      if (existing === bycliRoot) needsUpdate = false;
    } catch { /* doesn't exist */ }
    if (needsUpdate) {
      ...
      await fs.promises.symlink(bycliRoot, symlinkPath, symlinkType);
    }
  } catch (err) {
 ...
 *
 * With smart sync, ~/.bycli/clis/ only holds files that differ from the
  ...
  await Promise.all(files.map(async (file) => {
    ...
    if (file.endsWith('.js') && !file.endsWith('.d.js')) {
      ...
      log.warn(
        ...
        `Run "bycli plugin update ${site}" to re-transpile, or install esbuild.`
      );
    }
  }));
}
