...
async function captureNetworkItems(page: import('./types.js').IPage): Promise<BrowserNetworkItem[]> {
  ...
  const raw = await page.evaluate(`(function(){ var out = window.__bycli_net || []; window.__bycli_net = []; return JSON.stringify(out); })()`) as string;
  try {
    const parsed = JSON.parse(raw) as BrowserNetworkItem[];
    return parsed.map((item) => ({ ...item, timestamp: timestampFromRaw(item.timestamp) }));
  } catch {
    if (process.env.BYCLI_VERBOSE) log.warn(`[network] Failed to parse interceptor buffer: ${typeof raw === 'string' ? raw.slice(0, 200) : String(raw)}`);
    return [];
  }
}
 ...
 * ~/.bycli/sites/<site>/. Agents have a strong tendency to forget to write
...
function applyVerbose(opts: { verbose?: boolean }): void {
  if (opts.verbose) process.env.BYCLI_VERBOSE = '1';
}
