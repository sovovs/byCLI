/**
 * Plugin manifest: reads and validates bycli-plugin.json files.
 *
 * Supports two modes:
...
export interface SubPluginEntry {
  ...
  version?: string;
  description?: string;
  /** Semver range for bycli compatibility (overrides top-level). */
  bycli?: string;
...
export interface PluginManifest {
  ...
  version?: string;
  /** Semver range for bycli compatibility, e.g. ">=1.0.0". */
  bycli?: string;
...
export const MANIFEST_FILENAME = 'bycli-plugin.json';

// ── Read / Validate ─────────────────────────────────────────────────────────

/**
 * Read and parse bycli-plugin.json from a directory.
...
/**
 * Check if the current bycli version satisfies a semver range string.
 *
