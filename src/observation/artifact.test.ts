...
describe('observation artifact', () => {
  ...
  beforeEach(() => {
    baseDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bycli-trace-'));
  });
  ...
  it('writes artifacts under profile-scoped trace directory', () => {
    ...
    expect(summary).toContain('bycliVersion:');
    ...
    expect(receipt).toMatchObject({
      schemaVersion: 1,
      bycliVersion: expect.any(String),
      traceId: 'trace-1',
      traceDir: result.dir,
  ...
  it('builds a compact trace receipt', () => {
    const receipt = buildTraceReceipt({
      traceId: 'trace-1',
      dir: '/tmp/bycli/profiles/work/traces/trace-1',
      summaryPath: '/tmp/bycli/profiles/work/traces/trace-1/summary.md',
      receiptPath: '/tmp/bycli/profiles/work/traces/trace-1/receipt.json',
    ...
    expect(receipt).toMatchObject({
      ...
      bycliVersion: expect.any(String),
      traceId: 'trace-1',
      traceDir: '/tmp/bycli/profiles/work/traces/trace-1',
      receiptPath: '/tmp/bycli/profiles/work/traces/trace-1/receipt.json',
      status: 'failure',
