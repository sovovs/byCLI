...
export function formatRootAdapterHelpText(groups: RootAdapterGroups): string {
  ...
  lines.push("Run 'bycli list' for full command details, or 'bycli <site> --help' to inspect one site.");
  lines.push("Agent tip: use 'bycli <site> --help -f yaml' for all command args/options in one structured response.");
  lines.push('');
  return lines.join('\n');
}
...
function formatUsage(cmd: CliCommand): string {
  const positionalText = formatPositionals(positionals(cmd));
  return `bycli ${cmd.site} ${cmd.name}${positionalText ? ` ${positionalText}` : ''} [options]`;
}

function compactCommand(cmd: CliCommand): Record<string, unknown> {
  return {
    name: cmd.name,
    command: `bycli ${cmd.site} ${cmd.name}`,
...
export function formatSiteHelpText(site: string, commands: readonly CliCommand[]): string {
  const unique = [...new Map(commands.map(cmd => [fullName(cmd), cmd])).values()]
    .sort((a, b) => a.name.localeCompare(b.name));
  const lines: string[] = [
    `Usage: bycli ${site} <command> [args] [options]`,
    '',
