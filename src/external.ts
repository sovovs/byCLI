...
export interface ExternalCliConfig {
  /** User-facing byCLI subcommand and, by default, the executable name. */
  name: string;
  binary: string;
  /**
...
function getUserRegistryPath(): string {
  ...
  return path.join(home, '.bycli', 'external-clis.yaml');
}
...
export function executeExternalCli(name: string, args: string[], preloaded?: ExternalCliConfig[]): void {
  ...
  if (!isBinaryInstalled(cli.binary)) {
    ...
    if (!isBinaryInstalled(cli.binary)) {
      log.error(`Installed '${cli.name}', but '${cli.binary}' is still not available on PATH.`);
      log.info(
        `Check that the installer created '${cli.binary}', then reopen your terminal or add its install directory to PATH.`
      );
      if (cli.homepage) log.info(`See: ${cli.homepage}`);
      process.exitCode = EXIT_CODES.SERVICE_UNAVAIL;
      return;
