 ...
 * ## Exit codes
 *
 * bycli follows Unix conventions (sysexits.h) for process exit codes:
 *
 *   0   Success
...
export class TimeoutError extends CliError {
  constructor(label: string, seconds: number, hint?: string) {
    super(
      ...
      hint ?? 'Try again, or increase timeout with --timeout <seconds> (or BYCLI_BROWSER_COMMAND_TIMEOUT for the global default)',
      EXIT_CODES.TEMPFAIL,
    );
  }
}
