...
export function analyzeSite(
  ...
  if (antiBot.detected) {
    ...
    next = 'Pick the most specific JSON endpoint from `bycli browser network` and try a bare Node fetch with cookies; escalate to browser-context fetch only if blocked.';
  } else if (pattern.pattern === 'B') {
    next = 'Read the SSR global via `bycli browser eval "JSON.stringify(window.__INITIAL_STATE__ ?? window.__NUXT__ ?? window.__NEXT_DATA__ ?? window.__APOLLO_STATE__)"` — no API needed.';
  } else if (pattern.pattern === 'C') {
    next = 'No API visible — use SSR HTML scrape (e.g. `bycli browser extract`) against the rendered page.';
    ...
    next = 'No strong signal. Manually inspect `bycli browser network --all` and pick a pattern.';
  }
