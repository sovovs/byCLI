...
export abstract class BasePage implements IPage {
  ...
  protected async tryCdpOnResolvedElement(method: 'DOM.focus' | 'DOM.scrollIntoViewIfNeeded'): Promise<boolean> {
    ...
    const markerAttr = 'data-bycli-cdp-target';
  ...
  async setChecked(ref: string, checked: boolean, opts: ResolveOptions = {}): Promise<SetCheckedResult> {
    ...
    if (before?.ok !== true) {
      throw new TargetError({
        ...
        hint: 'Use `bycli browser state` or `browser find` to pick an input[type=checkbox], input[type=radio], or role=checkbox/switch target.',
      });
    }
    if (before.disabled) {
      throw new TargetError({
  ...
  async uploadFiles(ref: string, files: string[], opts: ResolveOptions = {}): Promise<UploadFilesResult> {
    ...
    const resolved = await runResolve(this, ref, opts);
    const markerAttr = 'data-bycli-upload-target';
    ...
    try {
      ...
      if (!marked) {
        throw new TargetError({
          ...
          hint: 'Use `bycli browser find --css "input[type=file]"` or inspect `compound` output from browser state/find.',
        });
      }
  ...
  async drag(
    ...
    try {
      ...
      const endpoints = await this.evaluate(`
        (() => {
          const sourceEl = window.__bycli_drag_source;
      ...
      await this.evaluate('delete window.__bycli_drag_source').catch(() => {});
    }
  }
  ...
  async fillText(ref: string, text: string, opts: ResolveOptions = {}): Promise<FillTextResult> {
    ...
    if (preparation?.ok !== true) {
      throw new TargetError({
        ...
        hint: 'Use `bycli browser state` to pick an editable target, or use `browser type` for keyboard-like interactions.',
      });
    }
  ...
  async snapshot(opts: SnapshotOptions = {}): Promise<unknown> {
    ...
    try {
      ...
      try {
        const hashes = await this.evaluate('window.__bycli_prev_hashes') as string | null;
