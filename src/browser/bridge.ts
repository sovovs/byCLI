  ...
  private async _ensureDaemon(timeoutSeconds?: number, contextId?: string): Promise<void> {
    ...
    if (isStale) {
      ...
      if (!portReleased) {
        ...
        throw new BrowserConnectError(
          ...
          '  Run manually: bycli daemon stop && bycli doctor',
          'daemon-not-running',
        );
      }
    ...
    if (!staleDaemonReplaced && health.state === 'profile-required') {
      throw new BrowserConnectError(
        ...
        'Select one with --profile <name>, BYCLI_PROFILE=<name>, or bycli profile use <name>.\n' +
        'Run bycli profile list to see connected profiles.',
        'profile-required',
      );
    }
    ...
    if (!staleDaemonReplaced && health.state === 'profile-disconnected') {
      ...
      throw new BrowserConnectError(
        ...
        'Open the matching Chrome profile and make sure the byCLI extension is enabled, or choose another profile with bycli profile use <name>.',
    ...
    if (!staleDaemonReplaced && health.state === 'no-extension') {
      ...
      if (process.env.BYCLI_VERBOSE || process.stderr.isTTY) {
        ...
        process.stderr.write('   Make sure Chrome or Chromium is open and the byCLI extension is enabled.\n');
      }
      ...
      throw new BrowserConnectError(
        ...
        'If the extension is installed, try: bycli daemon stop && bycli doctor\n' +
        'If not installed:\n' +
        '  1. Download: https://github.com/sovovs/byCLI/releases\n' +
