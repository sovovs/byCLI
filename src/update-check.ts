...
const CACHE_DIR = path.join(os.homedir(), '.bycli');
const CACHE_FILE = path.join(CACHE_DIR, 'update-check.json');
const CHECK_INTERVAL_MS = 24 * 60 * 60 * 1000; // 24h
const EXTENSION_STALE_MS = 7 * 24 * 60 * 60 * 1000; // 7d
const NPM_REGISTRY_URL = 'https://registry.npmjs.org/@sovovs/bycli/latest';
const GITHUB_RELEASES_URL = 'https://api.github.com/repos/sovovs/byCLI/releases?per_page=20';
...
function buildUpdateNotices({ cliVersion, cache, now }: NoticeInputs): NoticeLines {
  ...
  if (cache.latestVersion && isNewer(cache.latestVersion, cliVersion)) {
    lines.cli =
      ...
      `  Run: npm install -g @sovovs/bycli\n`;
  }
  ...
  if (
    ...
    lines.extension =
      ...
      `  Download: https://github.com/sovovs/byCLI/releases\n`;
  }
  return lines;
}
...
function extractLatestExtensionVersionFromReleases(releases: GitHubRelease[]): string | undefined {
  for (const release of releases) {
    for (const asset of release.assets ?? []) {
      const assetMatch = asset.name.match(/^bycli-extension-v(.+)\.zip$/);
...
async function fetchLatestExtensionVersion(): Promise<string | undefined> {
  try {
    ...
    const res = await fetch(GITHUB_RELEASES_URL, {
      ...
      headers: { 'User-Agent': `bycli/${PKG_VERSION}`, Accept: 'application/vnd.github+json' },
    });
    clearTimeout(timer);
  ...
  void (async () => {
    try {
      ...
      const res = await fetch(NPM_REGISTRY_URL, {
        signal: controller.signal,
        headers: { 'User-Agent': `bycli/${PKG_VERSION}` },
      });
      clearTimeout(timer);
      if (!res.ok) return;
 ...
 * Used by `bycli doctor` to report extension updates.
 */
