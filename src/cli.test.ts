...
describe('createProgram root help descriptions', () => {
  ...
  it('keeps leaf command descriptions unchanged', () => {
    ...
    expect(descriptionFor(program, 'doctor')).toBe('Diagnose bycli browser bridge connectivity');
  });
  ...
  it('keeps site adapters out of root commands and lists sites in the root help tail', () => {
    ...
    try {
      ...
      expect(help).toContain("bycli <site> --help -f yaml");
  ...
  it('exposes external_clis / app_adapters / site_adapters in structured help', () => {
    ...
    try {
      ...
      const program = createProgram('', '');
      process.argv = ['node', 'bycli', '--help', '-f', 'yaml'];
  ...
  it('renders per-site structured help with all commands, access, args, and examples', () => {
    ...
    try {
      ...
      process.argv = ['node', 'bycli', 'bilibili', '--help', '-f', 'yaml'];
  ...
  it('separates command args from common options in structured help', () => {
    ...
    try {
      ...
      process.argv = ['node', 'bycli', 'bilibili', 'video', '--help', '-f', 'yaml'];
      const data = yaml.load(command!.helpInformation()) as any;

      expect(data.usage).toBe('bycli bilibili video <bvid> [options]');
  ...
  it('renders browser namespace structured help from Commander metadata', () => {
    ...
    try {
      ...
      process.argv = ['node', 'bycli', 'browser', '--session', 'test', '--help', '-f', 'yaml'];
      const data = yaml.load(browser!.helpInformation()) as any;

      expect(data.namespace).toBe('browser');
      expect(data.command).toBe('bycli browser');
      ...
      expect(data.usage).toBe('bycli browser <session> <command> [options]');
      ...
      expect(click).toMatchObject({
        command: 'bycli browser <session> click',
        usage: 'bycli browser <session> click [target] [options]',
      ...
      expect(tabList).toMatchObject({
        command: 'bycli browser <session> tab list',
        usage: 'bycli browser <session> tab list [options]',
        command_options: [],
      });

      const getText = data.commands.find((cmd: any) => cmd.name === 'get text');
      expect(getText).toMatchObject({
        command: 'bycli browser <session> get text',
        positionals: [{ name: 'target' }],
      });
      expect(data.structured_help).toMatchObject({
        formats: ['yaml', 'json'],
        usage: 'bycli browser --help -f yaml',
      });
    } finally {
      process.argv = argv;
    }
  ...
  it('renders nested browser parent structured help for a subtree', () => {
    ...
    try {
      ...
      process.argv = ['node', 'bycli', 'browser', '--session', 'test', 'tab', '--help', '-f', 'yaml'];
      const data = yaml.load(tab!.helpInformation()) as any;

      expect(data).toMatchObject({
        namespace: 'browser',
        group: 'tab',
        command: 'bycli browser <session> tab',
        usage: 'bycli browser <session> tab <command> [args] [options]',
        command_count: 4,
      });
      ...
      expect(data.commands.find((cmd: any) => cmd.name === 'tab close')).toMatchObject({
        command: 'bycli browser <session> tab close',
        usage: 'bycli browser <session> tab close [targetId] [options]',
      ...
      expect(data.structured_help).toMatchObject({
        usage: 'bycli browser <session> tab --help -f yaml',
      });
    } finally {
      process.argv = argv;
    }
  ...
  it('renders browser command structured help without needing the full namespace dump', () => {
    ...
    try {
      ...
      process.argv = ['node', 'bycli', 'browser', '--session', 'test', 'click', '--help', '-f', 'yaml'];
      ...
      expect(data).toMatchObject({
        ...
        positionals: [{ name: 'target' }],
        structured_help: {
          usage: 'bycli browser <session> click --help -f yaml',
        },
      });
  ...
  it('renders daemon namespace structured help with leaves and global options', () => {
    ...
    try {
      ...
      process.argv = ['node', 'bycli', 'daemon', '--help', '-f', 'yaml'];
      const data = yaml.load(daemon.helpInformation()) as any;

      expect(data).toMatchObject({
        namespace: 'daemon',
        command: 'bycli daemon',
        usage: 'bycli daemon <command> [args] [options]',
        description: 'Manage the bycli daemon',
        command_count: 3,
        namespace_options: [],
        structured_help: { usage: 'bycli daemon --help -f yaml' },
      });
  ...
  it('renders plugin namespace structured help with positional + option leaves', () => {
    ...
    try {
      ...
      process.argv = ['node', 'bycli', 'plugin', '--help', '-f', 'yaml'];
      const data = yaml.load(plugin.helpInformation()) as any;

      expect(data).toMatchObject({
        namespace: 'plugin',
        command: 'bycli plugin',
        description: 'Manage bycli plugins',
        namespace_options: [],
      });
      ...
      expect(update).toMatchObject({
        usage: 'bycli plugin update [name] [options]',
  ...
  it('renders adapter namespace structured help preserving original description after applyRootSubcommandSummaries', () => {
    ...
    try {
      ...
      process.argv = ['node', 'bycli', 'adapter', '--help', '-f', 'yaml'];
      ...
      expect(reset).toMatchObject({
        usage: 'bycli adapter reset [site] [options]',
  ...
  it('renders profile namespace structured help including required positionals', () => {
    ...
    try {
      ...
      process.argv = ['node', 'bycli', 'profile', '--help', '-f', 'yaml'];
      ...
      expect(rename).toMatchObject({
        usage: 'bycli profile rename <contextId> <alias> [options]',
        positionals: [
...
describe('resolveBrowserVerifyInvocation', () => {
  it('prefers the built entry declared in package metadata', () => {
    ...
    expect(resolveBrowserVerifyInvocation({
      projectRoot,
      readFile: () => JSON.stringify({ bin: { bycli: 'dist/src/main.js' } }),
...
describe('browser verify', () => {
  ...
  it('passes --trace through to the adapter subprocess', async () => {
    ...
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'bycli-browser-verify-trace-'));
    process.env.HOME = fakeHome;
    process.env.USERPROFILE = fakeHome;

    try {
      const adapterDir = path.join(fakeHome, '.bycli', 'clis', 'hn');
      fs.mkdirSync(adapterDir, { recursive: true });
      fs.writeFileSync(path.join(adapterDir, 'top.js'), 'export default {};\n', 'utf-8');

      const program = createProgram('', '');
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'verify', 'hn/top', '--no-fixture', '--trace', 'retain-on-failure']);
  ...
  it('uses --seed-args when no fixture args exist', async () => {
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'bycli-browser-verify-seed-'));
    process.env.HOME = fakeHome;
    process.env.USERPROFILE = fakeHome;
    ...
    try {
      ...
      const program = createProgram('', '');
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'verify', 'hn/top', '--no-fixture', '--seed-args', 'bycli-verify']);

      expect(mockExecFileSync).toHaveBeenCalledTimes(1);
      const [, execArgs] = mockExecFileSync.mock.calls[0] as [string, string[]];
      expect(execArgs.slice(-5)).toEqual(['hn', 'top', 'bycli-verify', '--format', 'json']);
    } finally {
      if (originalHome === undefined) delete process.env.HOME;
      else process.env.HOME = originalHome;
      if (originalUserProfile === undefined) delete process.env.USERPROFILE;
  ...
  it('writes --seed-args into a starter fixture', async () => {
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'bycli-browser-verify-write-seed-'));
    process.env.HOME = fakeHome;
    process.env.USERPROFILE = fakeHome;
    ...
    try {
      ...
      const program = createProgram('', '');
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'verify', 'hn/top', '--write-fixture', '--seed-args', 'bycli-verify']);

      const fixtureFile = path.join(fakeHome, '.bycli', 'sites', 'hn', 'verify', 'top.json');
      const fixture = JSON.parse(fs.readFileSync(fixtureFile, 'utf-8'));
      expect(fixture.args).toEqual(['bycli-verify']);
  ...
  it('fails before fixture handling when output row shape is not agent-native', async () => {
    const originalHome = process.env.HOME;
    const originalUserProfile = process.env.USERPROFILE;
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'bycli-browser-verify-shape-'));
    process.env.HOME = fakeHome;
    process.env.USERPROFILE = fakeHome;
    ...
    try {
      ...
      const program = createProgram('', '');
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'verify', 'hn/top', '--no-fixture']);
...
describe('profile list', () => {
  ...
  it('reports stale daemon instead of no profiles when status lacks profile support', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'profile', 'list']);

    const output = stdoutSpy.mock.calls.flat().join('\n');
    expect(output).toContain('stale');
    expect(output).toContain('bycli daemon restart');
  ...
  it('keeps the empty profile message for current daemon status with no profiles', async () => {
    ...
    expect(output).not.toContain('bycli daemon restart');
  });
});
...
describe('browser tab targeting commands', () => {
  ...
  beforeEach(() => {
    process.exitCode = undefined;
    process.env.BYCLI_CACHE_DIR = fs.mkdtempSync(path.join(os.tmpdir(), 'bycli-browser-tab-state-'));
    consoleLogSpy.mockClear();
    stderrSpy.mockClear();
    mockBrowserConnect.mockClear();
    mockBrowserClose.mockReset().mockResolvedValue(undefined);
    delete process.env.BYCLI_WINDOW;
  ...
  it('binds the current Chrome tab into a browser session', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'bind']);
  ...
  it('requires an explicit session for browser commands', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', 'state']);
  ...
  it('runs browser commands against an explicit session', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'state']);
  ...
  it('passes browser --window through Commander options without relying on env pre-processing', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', '--window', 'background', 'state']);
  ...
  it('passes the opt-in AX source to browser state', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'state', '--source', 'ax']);
  ...
  it('prints DOM vs AX snapshot metrics without changing default state output', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'state', '--compare-sources']);
  ...
  it('rejects unknown browser state sources before touching the page', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'state', '--source', 'magic']);
  ...
  it('captures annotated screenshots through the visual ref overlay path', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'screenshot', '--annotate']);
  ...
  it('allows history navigation in a bound session', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'back']);
  ...
  it('unbinds a session through the daemon close-window command', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'unbind']);
  ...
  it('accepts JavaScript dialogs through the browser dialog command', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'dialog', 'accept', '--text', 'ok']);
  ...
  it('emits a structured error when a browser action is blocked by a JavaScript dialog', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'eval', 'document.title']);
  ...
  it('binds browser commands to an explicit target tab via --tab', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'eval', '--tab', 'tab-2', 'document.title']);
  ...
  it('rejects an explicit --tab target that is no longer in the current session', async () => {
    ...
    const program = createProgram('', '');
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'eval', '--tab', 'tab-stale', 'document.title']);
  ...
  it('lists tabs with target IDs via browser tab list', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'tab', 'list']);
  ...
  it('creates a new tab and prints its target ID', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'tab', 'new', 'https://three.example']);
  ...
  it('prints the resolved target ID when browser open creates or navigates a tab', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'open', 'https://example.com']);
  ...
  it('lists cross-origin frames via browser frames', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'frames']);
  ...
  it('routes browser eval --frame through frame-targeted evaluation', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'eval', '--frame', '0', 'document.title']);
  ...
  it('persists an explicitly selected tab as the default target for later untargeted commands', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'tab', 'select', 'tab-2']);
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'eval', 'document.title']);
  ...
  it('clears a saved default target when it is no longer present in the current session', async () => {
    const cacheDir = String(process.env.BYCLI_CACHE_DIR);
    const program = createProgram('', '');
  ...
  it('clears the persisted default target when that tab is closed', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'tab', 'select', 'tab-2']);
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'tab', 'close', 'tab-2']);
  ...
  it('rejects closing a stale tab target ID that is no longer in the current session', async () => {
    ...
    const program = createProgram('', '');
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'tab', 'close', 'tab-stale']);
  ...
  it('browser analyze merges HttpOnly cookie names from page.getCookies and drains stale capture before verdict', async () => {
    ...
    const program = createProgram('', '');
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'analyze', 'https://target.example/']);
  ...
  it('browser analyze falls back to interceptor buffer when network capture is unsupported', async () => {
    ...
    browserState.page = {
      ...
      evaluate: vi.fn().mockImplementation(async (arg: string) => {
        ...
        if (typeof arg === 'string' && arg.includes('window.__bycli_net = []')) {
          bufferReads += 1;
          if (bufferReads === 1) {
  ...
  it('browser wait xhr starts capture, injects interceptor on fallback, and ignores stale ring entries', async () => {
    ...
    const program = createProgram('', '');
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'wait', 'xhr', '/api/target', '--timeout', '900']);

    const out = lastJsonLog();
    expect(browserState.page?.startNetworkCapture).toHaveBeenCalledTimes(1);
    expect(browserState.page?.evaluate).toHaveBeenCalledWith(expect.stringContaining('window.__bycli_net'));
    expect(browserState.page?.readNetworkCapture).toHaveBeenCalledTimes(2);
  ...
  it('browser wait download delegates to the Browser Bridge download observer', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'wait', 'download', 'receipt', '--timeout', '900']);
...
describe('browser network command', () => {
  ...
  beforeEach(() => {
    process.exitCode = undefined;
    process.env.BYCLI_CACHE_DIR = fs.mkdtempSync(path.join(os.tmpdir(), 'bycli-browser-net-'));
    consoleLogSpy.mockClear();
    mockBrowserConnect.mockClear();
    mockBrowserClose.mockReset().mockResolvedValue(undefined);
  ...
  it('emits JSON with shape previews and persists the capture to disk', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network']);
  ...
  it('uses the selected browser session for network cache scope', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'custom', 'network']);
  ...
  it('--all includes static resources that the default filter drops', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--all']);
  ...
  it('--failed and --since filter captured entries by status and time window', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--since', '120s', '--failed']);
  ...
  it('--raw emits full bodies inline for every entry', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--raw']);
  ...
  it('--detail <key> returns the full body for the requested entry', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network']);
    consoleLogSpy.mockClear();
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--detail', 'UserTweets']);
  ...
  it('--detail reports key_not_found with the list of available keys', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network']);
    consoleLogSpy.mockClear();
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--detail', 'NopeOp']);
  ...
  describe('--filter', () => {
    ...
    it('narrows entries to those whose shape has ALL named fields', async () => {
      const program = createProgram('', '');
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--filter', 'author,text,likes']);
    ...
    it('matches container segments too, not just leaf names (any-segment rule)', async () => {
      const program = createProgram('', '');
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--filter', 'data,items']);
    ...
    it('drops entries that are missing any required field (AND semantics)', async () => {
      const program = createProgram('', '');
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--filter', 'author,followers']);
    ...
    it('returns empty entries (not an error) when nothing matches', async () => {
      const program = createProgram('', '');
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--filter', 'nonexistent_field']);
    ...
    it('is case-sensitive so agents do not conflate `Id` with `id`', async () => {
      const program = createProgram('', '');
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--filter', 'Data']);
    ...
    it('persists the full (unfiltered) capture so --detail lookups still find filtered-out keys', async () => {
      ...
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--filter', 'author,text,likes']);
      consoleLogSpy.mockClear();
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--detail', 'UserProfile']);
    ...
    it('composes with --raw: entries keep full bodies, filter still narrows', async () => {
      const program = createProgram('', '');
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--filter', 'author', '--raw']);
    ...
    it('reports invalid_filter for empty value', async () => {
      const program = createProgram('', '');
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--filter', '']);
    ...
    it('rejects --filter combined with --detail as invalid_args', async () => {
      const program = createProgram('', '');
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--filter', 'author', '--detail', 'UserTweets']);
  ...
  describe('body truncation signals', () => {
    ...
    it('--detail surfaces body_truncated + body_full_size when capture had to cap the body', async () => {
      ...
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network']);
      consoleLogSpy.mockClear();
      await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'network', '--detail', 'GET api.example.com/huge']);
    ...
    it('--max-body caps the emitted body and marks body_truncation_reason = max-body', async () => {
      ...
      await program.parseAsync([
        'node', 'bycli', 'browser', '--session', 'test', 'network',
...
describe('browser console command', () => {
  ...
  it('filters console messages by level and time window', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'console', '--level', 'error', '--since', '120s']);
...
describe('browser get html command', () => {
  ...
  beforeEach(() => {
    process.exitCode = undefined;
    process.env.BYCLI_CACHE_DIR = fs.mkdtempSync(path.join(os.tmpdir(), 'bycli-html-'));
    consoleLogSpy.mockClear();
    mockBrowserConnect.mockClear();
    mockBrowserClose.mockReset().mockResolvedValue(undefined);
  ...
  it('returns full outerHTML by default with no truncation', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'html']);
  ...
  it('caps output with --max and prepends a visible truncation marker', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'html', '--max', '100']);

    const out = String(lastLogArg());
    expect(out.startsWith('<!-- bycli: truncated 100 of')).toBe(true);
  ...
  it('rejects negative --max with invalid_max error', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'html', '--max', '-1']);
  ...
  it('rejects fractional --max with invalid_max error', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'html', '--max', '1.5']);
  ...
  it('rejects non-numeric --max (e.g. "10abc") with invalid_max error', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'html', '--max', '10abc']);
  ...
  it('--as json returns structured tree envelope', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'html', '--selector', '.hero', '--as', 'json']);
  ...
  it('--as json emits selector_not_found when matched is 0', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'html', '--selector', '.missing', '--as', 'json']);
  ...
  it('raw mode emits selector_not_found when the selector matches nothing', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'html', '--selector', '.missing']);
  ...
  it('raw mode emits invalid_selector when the page rejects the selector syntax', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'html', '--selector', '##$@@']);
  ...
  it('--as json emits invalid_selector when the page rejects the selector syntax', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'html', '--selector', '##$@@', '--as', 'json']);
  ...
  it('rejects unknown --as format with invalid_format error', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'html', '--as', 'yaml']);
...
function installSelectorFirstTestHarness(label: string, pageOverrides: () => Partial<IPage>) {
  ...
  beforeEach(() => {
    process.exitCode = undefined;
    process.env.BYCLI_CACHE_DIR = fs.mkdtempSync(path.join(os.tmpdir(), `bycli-${label}-`));
    consoleLogSpy.mockClear();
    mockBrowserConnect.mockClear();
    mockBrowserClose.mockReset().mockResolvedValue(undefined);
...
describe('browser find command', () => {
  ...
  it('returns a {matches_n, entries} envelope for a matching selector', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'find', '--css', '.btn']);
  ...
  it('finds elements by semantic role/name without requiring CSS', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'find', '--role', 'button', '--name', 'Save']);
  ...
  it('forwards --limit / --text-max into the generated JS', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'find', '--css', '.btn', '--limit', '3', '--text-max', '20']);
  ...
  it('emits invalid_selector envelope when the page rejects selector syntax', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'find', '--css', '>>>']);
  ...
  it('emits selector_not_found envelope when the selector matches nothing', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'find', '--css', '.missing']);
  ...
  it('rejects missing --css with usage_error (no evaluate call)', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'find']);
  ...
  it('rejects malformed --limit with usage_error (no evaluate call)', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'find', '--css', '.btn', '--limit', 'abc']);
...
describe('browser get text/value/attributes commands', () => {
  ...
  it('emits {value, matches_n, match_level} envelope for a numeric ref', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'text', '7']);
  ...
  it('resolves a semantic locator to a ref before get text', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'text', '--role', 'button', '--name', 'Save']);
  ...
  it('reports matches_n on multi-match CSS (read path: first match wins)', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'text', '.btn']);
  ...
  it('parses the attributes payload back into a real object', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'attributes', '#nav']);
  ...
  it('propagates selector_not_found from the resolver as an error envelope', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'text', '.missing']);
  ...
  it('forwards --nth into the resolver opts and reports matches_n', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'value', '.btn', '--nth', '1']);
  ...
  it('rejects malformed --nth with usage_error before touching the page', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'get', 'text', '.btn', '--nth', 'abc']);
...
describe('browser click/type commands', () => {
  ...
  it('emits {clicked, target, matches_n, match_level} on success', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'click', '#save']);
  ...
  it('clicks a unique semantic locator without a prior state call', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'click', '--role', 'button', '--name', 'Submit']);
  ...
  it('rejects ambiguous semantic locators before write actions', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'click', '--role', 'button', '--name', 'Save']);
  ...
  it('hover: resolves a semantic locator before moving the mouse', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'hover', '--role', 'button', '--name', 'Settings']);
  ...
  it('check: resolves a semantic locator before setting checked state', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'check', '--role', 'checkbox', '--name', 'Accept']);
  ...
  it('upload: treats the first positional as a file when using semantic locator flags', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'bycli-upload-semantic-'));
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'upload', '--role', 'button', '--name', 'Upload receipt', file]);
  ...
  it('type: treats the first positional as text when using semantic locator flags', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'type', '--label', 'Email', 'me@example.com']);
  ...
  it('fill: treats the first positional as text when using semantic locator flags', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'fill', '--label', 'Email', 'me@example.com']);
  ...
  it('surfaces match_level=stable when resolver falls back to fingerprint match', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'click', '7']);
  ...
  it('forwards --nth as ResolveOptions.nth to page.click', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'click', '.btn', '--nth', '2']);
  ...
  it('surfaces selector_ambiguous from page.click as an error envelope', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'click', '.btn']);
  ...
  it('surfaces selector_nth_out_of_range from page.click as an error envelope', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'click', '.btn', '--nth', '99']);
  ...
  it('rejects malformed --nth on click with usage_error before touching the page', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'click', '.btn', '--nth', 'abc']);
  ...
  it('hover: delegates to page.hover and emits a structured envelope', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'hover', '.menu', '--nth', '1']);
  ...
  it('focus: delegates to page.focus and reports whether the element took focus', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'focus', '7']);
  ...
  it('dblclick: delegates to page.dblClick and emits a structured envelope', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'dblclick', '#row']);
  ...
  it('check: ensures target is checked through page.setChecked', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'check', '.todo', '--nth', '1']);
  ...
  it('uncheck: ensures target is unchecked through page.setChecked', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'uncheck', '#subscribe']);
  ...
  it('upload: validates local files and delegates to page.uploadFiles', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'bycli-upload-'));
    const file = path.join(dir, 'receipt.pdf');
    fs.writeFileSync(file, 'pdf');
    (browserState.page!.uploadFiles as any).mockResolvedValueOnce({
      uploaded: true,
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'upload', '#file', file]);
  ...
  it('upload: rejects missing files before touching the page', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'upload', '#file', '/tmp/bycli-missing-file']);
  ...
  it('drag: delegates to page.drag and forwards source/target nth options', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'drag', '.card', '.lane', '--from-nth', '2', '--to-nth', '1']);
  ...
  it('drag: rejects malformed --from-nth before touching the page', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'drag', '.card', '.lane', '--from-nth', 'abc']);
  ...
  it('type: clicks, waits, then typeText — emits {typed, text, target, matches_n, match_level, autocomplete}', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'type', '#q', 'hello']);
  ...
  it('type: waits an extra 0.4s when the input reports autocomplete=true', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'type', '#q', 'hi']);
  ...
  it('type: surfaces match_level=reidentified when ref had to be reidentified by fingerprint', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'type', '9', 'hi']);
  ...
  it('type: forwards --nth to both click and typeText', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'type', '.field', 'x', '--nth', '3']);
  ...
  it('fill: delegates exact raw text to page.fillText and emits verification details', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'fill', '#msg', 'line1\\n/ / raw']);
  ...
  it('fill: sets a non-zero exit code when verification fails', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'fill', '#msg', 'expected']);
  ...
  it('fill: forwards --nth to page.fillText', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'fill', '.field', 'x', '--nth', '2']);
...
describe('browser select command', () => {
  ...
  it('emits {selected, target, matches_n, match_level} on success', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'select', '#country', 'US']);
  ...
  it('maps "Not a <select>" to a not_a_select error envelope', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'select', '#not-select', 'US']);
  ...
  it('maps missing-option failures to an option_not_found envelope with available list', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'select', '#country', 'XX']);
  ...
  it('select: treats the first positional as option when using semantic locator flags', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'select', '--label', 'Country', 'Uruguay']);
  ...
  it('surfaces selector_ambiguous from the resolver before calling selectResolvedJs', async () => {
    ...
    await program.parseAsync(['node', 'bycli', 'browser', '--session', 'test', 'select', '.dropdown', 'US']);
