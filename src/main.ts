#!/usr/bin/env node
/**
 * bycli — Make any website your CLI. AI-powered.
 */
...
if (typeof (globalThis as { Bun?: unknown }).Bun === 'undefined' && !isSupportedNodeVersion(process.version)) {
  process.stderr.write(
    [
      `byCLI requires Node.js >= ${MIN_SUPPORTED_NODE_MAJOR}.0.0.`,
...
//    (~/.bycli/node_modules/ vs ~/.bycli/clis/ + adapter-manifest.json).
...
// Rewrite `bycli browser <session> <subcommand> ...` so commander (which
// can't combine a parent positional with subcommand dispatch) sees the internal
// `--session <name>` flag form. Also refuses the retired `bycli browser
