 ...
 *
 * Plugins live in ~/.bycli/plugins/<name>/.
 * Monorepo clones live in ~/.bycli/monorepos/<repo-name>/.
...
export function getLockFilePath(): string {
  return path.join(getHomeDir(), '.bycli', 'plugins.lock.json');
}

/** Monorepo clones directory: ~/.bycli/monorepos/ */
export function getMonoreposDir(): string {
  return path.join(getHomeDir(), '.bycli', 'monorepos');
}
...
export interface PluginInfo {
  ...
  monorepoName?: string;
  /** Description from bycli-plugin.json. */
  description?: string;
}
 ...
 * filesystems (e.g. /tmp → ~/.bycli). In that case we copy then remove.
 */
...
function cloneRepoToTemp(cloneUrl: string): string {
  const tmpCloneDir = path.join(
    os.tmpdir(),
    `bycli-clone-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`,
  );
...
export function validatePluginStructure(pluginDir: string): ValidationResult {
  ...
  if (hasTs) {
    ...
    if (!fs.existsSync(pkgJsonPath)) {
      errors.push('Plugin contains .ts files but no package.json. A package.json with "type": "module" and "@sovovs/bycli" peer dependency is required for TS plugins.');
    } else {
      try {
...
function finalizePluginRuntime(pluginDir: string): void {
  // Symlink host bycli so TS plugins resolve '@sovovs/bycli/registry'
  // against the running host, not a stale npm-published version.
  linkHostBycli(pluginDir);
...
export function installPlugin(source: string): string | string[] {
  ...
  return withTempClone(parsed.cloneUrl!, (tmpCloneDir) => {
    ...
    if (manifest?.bycli && !checkCompatibility(manifest.bycli)) {
      throw new Error(
        `Plugin requires bycli ${manifest.bycli}, but current version is incompatible.`
      );
    }
...
function installSinglePlugin(
  ...
  if (fs.existsSync(targetDir)) {
    throw new PluginError(`Plugin "${pluginName}" is already installed at ${targetDir}`, 'Use "bycli plugin uninstall" first, or pick a different name.');
  }
...
function installLocalPlugin(localPath: string, name: string): string {
  ...
  if (manifest?.bycli && !checkCompatibility(manifest.bycli)) {
    throw new PluginError(
      `Plugin requires bycli ${manifest.bycli}, but current version is incompatible.`,
      'Upgrade bycli to a compatible version.',
    );
  }
...
function installMonorepo(
  ...
  for (const { name, entry } of pluginsToInstall) {
    ...
    if (entry.bycli && !checkCompatibility(entry.bycli)) {
      log.warn(`Skipping "${name}": requires bycli ${entry.bycli}`);
      continue;
    }
...
function collectUpdatedMonorepoPlugins(
  ...
  for (const [pluginName, entry] of Object.entries(lock)) {
    ...
    if (manifestEntry.bycli && !checkCompatibility(manifestEntry.bycli)) {
      throw new Error(`Sub-plugin "${pluginName}" requires bycli ${manifestEntry.bycli}`);
    }
 ...
 * Reads bycli-plugin.json for description/version when available.
 */
...
/**
 * Symlink the host bycli package into a plugin's node_modules.
 * This ensures TS plugins resolve '@sovovs/bycli/registry' against
 * the running host installation rather than a stale npm-published version.
 */
function linkHostBycli(pluginDir: string): void {
  try {
    const hostRoot = resolveHostBycliRoot();

    const targetLink = path.join(pluginDir, 'node_modules', '@sovovs', 'bycli');
    ...
    log.debug(`Linked host bycli into plugin: ${targetLink} → ${hostRoot}`);
  } catch (err) {
    log.warn(`Failed to link host bycli into plugin: ${getErrorMessage(err)}`);
  }
}
...
function resolveHostBycliRoot(startFile = fileURLToPath(import.meta.url)): string {
 ...
 * Uses esbuild from the host bycli's node_modules for fast single-file transpilation.
 */
  ...
  try {
    ...
    if (!esbuildBin) {
      log.warn(
        ...
        'Install esbuild (`npm i -g esbuild`) or ensure it is available in the bycli host node_modules.'
      );
      return;
    }
...
export {
  resolveHostBycliRoot as _resolveHostBycliRoot,
