...
byCLI 将**任何网站**或 **Electron 应用**变成命令行界面 — Bilibili、知乎、小红书、Twitter/X、Reddit、YouTube、Antigravity 等 — 基于浏览器会话复用和 AI 原生发现。
...
```bash
bycli list                              # 查看所有命令
bycli hackernews top --limit 5          # 公开 API，无需浏览器
bycli bilibili hot --limit 5            # 浏览器命令
bycli zhihu hot -f json                 # JSON 输出
```
...
```bash
bycli bilibili hot -f table   # 默认：终端表格
bycli bilibili hot -f json    # JSON
bycli bilibili hot -f yaml    # YAML
...
bycli [Tab]          # 补全站点名称（bilibili、zhihu、twitter...）
bycli bilibili [Tab] # 补全命令（hot、search、me、download...）
```
...
- [扩展 byCLI：自定义命令、plugin 和 external CLI](/zh/guide/extending-bycli)
- [所有适配器](/zh/adapters/)
