...
byCLI 有五类扩展路径。按源码放在哪里、命令要如何共享来选。

| 目标 | 使用方式 | 源码位置 | 命令入口 |
|------|----------|----------|----------|
| 在自己的 Git repo 里写个人网站命令 | 本地 plugin | 你的项目目录，symlink 到 `~/.bycli/plugins/` | `bycli <plugin> <command>` |
| 快速写一个只在本机用的 adapter | User adapter | `~/.bycli/clis/<site>/<command>.js` | `bycli <site> <command>` |
| 本地修改官方 adapter | Adapter override | `~/.bycli/clis/<site>/` | `bycli <site> <command>` |
| 发布或安装第三方命令 | Plugin | Git repo，安装到 `~/.bycli/plugins/` | `bycli <plugin> <command>` |
| 包装已有本机 binary | External CLI | `~/.bycli/external-clis.yaml` | `bycli <tool> ...` |
...
`plugin install file://...` 会在 `~/.bycli/plugins/` 下创建 symlink。源码仍然留在你的项目目录，编辑和提交都在项目目录完成。

长期维护的自建命令推荐走这条路径。

## `~/.bycli/clis` 下的私人 adapter
...
`--seed-args` 只在 fixture 没有 `args` 时生效。fixture 写出后，`bycli browser verify` 会从 `~/.bycli/sites/<site>/verify/<command>.json` 读取 args。
...
`~/.bycli/clis/<site>/<command>.js` 会在本机覆盖同名 package adapter。`bycli browser verify <site>/<command>` 也会跑本地覆盖版本，所以本地 verify 通过不代表 package 里的 adapter 已经改好。

Package 里的 `cli-manifest.json` 只描述 bundled adapter。User adapter 是运行时发现的，不需要写 manifest。

把本地修复复制到仓库发 PR 后，merge 后要删除本地副本，或运行 `bycli adapter reset <site>`。否则本地文件会继续 shadow 后续 package 更新。`bycli doctor` 会在发现这种 shadowing 时给出 warning。
...
Git 托管的 plugin collection 可以在 `bycli-plugin.json` 里声明多个 sub-plugin：
...
如果命令已经是本机 binary，只想统一挂到 `bycli` 下，用 external CLI registration。
