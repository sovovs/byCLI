...
```typescript
/**
 * Generate JS to wait until window.__bycli_xhr has ≥1 captured response.
...
export function waitForCaptureJs(maxMs: number): string {
  return `
    new Promise((resolve, reject) => {
      ...
      const check = () => {
        if ((window.__bycli_xhr || []).length > 0) return resolve('captured');
...
describe('waitForCaptureJs', () => {
  it('returns a non-empty string', () => {
    ...
    expect(code).toContain('__bycli_xhr');
    expect(code).toContain('resolve');
    expect(code).toContain('reject');
  });

  it('resolves "captured" when __bycli_xhr is populated before deadline', async () => {
    const g = globalThis as any;
    g.__bycli_xhr = [];
    const code = waitForCaptureJs(1000);
    const promise = eval(code) as Promise<string>;
    g.__bycli_xhr.push({ data: 'test' });
    await expect(promise).resolves.toBe('captured');
    delete g.__bycli_xhr;
  });

  it('rejects when __bycli_xhr stays empty past deadline', async () => {
    const g = globalThis as any;
    g.__bycli_xhr = [];
  ...
  it('resolves immediately when __bycli_xhr already has data', async () => {
    const g = globalThis as any;
    g.__bycli_xhr = [{ data: 'already here' }];
    const code = waitForCaptureJs(1000);
...
The current `stepIntercept` uses `generateInterceptorJs`/`generateReadInterceptedJs` directly, writing to `__bycli_intercepted`. We unify this to use `page.installInterceptor()` (→ `__bycli_xhr`) + `page.waitForCapture()` + `page.getInterceptedRequests()`.
...
- **Layer 1 — `waitForCapture()`**: Fixes a correctness bug in INTERCEPT adapters where `wait(N)` (now DOM-stable-aware) could return before network captures arrive. Adds `waitForCapture(timeout)` to `IPage` — polls `window.__bycli_xhr` at 100ms intervals, resolves as soon as ≥1 capture exists. Applied to 36kr, twitter/search, followers, following, notifications, producthunt.
