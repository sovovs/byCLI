...
Note: The `/status` and `/shutdown` endpoints run inside the daemon process, which makes them hard to unit test in isolation. They are integration-tested via the `bycli daemon status/stop` commands in Task 6.
 ...
 *   bycli daemon status  — show daemon state
 *   bycli daemon stop    — graceful shutdown
 *   bycli daemon restart — stop + respawn
 */
...
git commit -m "feat(daemon): add bycli daemon status/stop/restart commands"
```
...
private async _ensureDaemon(timeoutSeconds?: number): Promise<void> {
  ...
  if (await isDaemonRunning()) {
    if (process.env.BYCLI_VERBOSE || process.stderr.isTTY) {
      process.stderr.write('⏳ Waiting for Chrome extension to connect...\n');
      process.stderr.write('   Make sure Chrome is open and the byCLI extension is enabled.\n');
    }
    ...
    throw new Error(
      ...
      'Please install and enable the bycli Browser Bridge extension in Chrome.',
    );
  }
  ...
  throw new Error(
    'Failed to start bycli daemon. Try running manually:\n' +
    `  node ${daemonPath}\n` +
