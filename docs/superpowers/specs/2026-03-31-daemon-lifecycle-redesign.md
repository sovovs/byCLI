...
byCLI's daemon auto-exits after 5 minutes of idle time. During typical development
...
4. Add `bycli daemon status/stop/restart` commands
...
Both endpoints require the same `X-byCLI` header as existing endpoints for CSRF
protection.
...
⏳ Waiting for Chrome extension to connect...
   Make sure Chrome is open and the byCLI extension is enabled.
```
...
- Existing `BYCLI_DAEMON_PORT` environment variable continues to work
...
- Integration test: `bycli daemon status/stop/restart` work correctly
