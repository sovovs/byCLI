...
Two-layer testing framework for `bycli browse` commands and the
Claude Code skill integration.

## Goal

Verify that `bycli browse` works reliably on real websites and that
...
Tests `bycli browse` commands directly on real websites. No LLM
...
```json
{
  "name": "hn-top-stories",
  "steps": [
    "bycli browse open https://news.ycombinator.com",
    "bycli browse eval \"JSON.stringify([...document.querySelectorAll('.titleline a')].slice(0,5).map(a=>({title:a.textContent,url:a.href})))\""
  ],
...
- `bycli browse close` between tasks for clean state
...
Spawns Claude Code with the bycli-browser skill to complete tasks
...
```bash
claude -p \
  --system-prompt "$(cat skills/bycli-browser/SKILL.md)" \
  --dangerously-skip-permissions \
  --allowedTools "Bash(bycli:*)" \
  --output-format json \
  "用 bycli browse 完成任务：Extract the top 5 stories from Hacker News with title and score. Start URL: https://news.ycombinator.com"
```
