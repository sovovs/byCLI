...
byCLI browser should become reliable for unknown SaaS workflows without
...
dropdowns worked with `agent-browser + Claude Code`, but byCLI often failed
to select the category.

## Source Findings

We compared byCLI against `vercel-labs/agent-browser` at source level, not
...
- `src/browser/base-page.ts` and `src/browser/target-resolver.ts`: generic
  ...
  change.
- `src/browser/dom-snapshot.ts`: byCLI observation is DOM-based. It emits
  useful compact refs, but refs do not have AX role/name/nth fallback semantics
  and are weaker across re-rendered portals.
- byCLI already has the right low-level plumbing: `IPage.cdp`,
...
- byCLI exposes fewer general browser primitives than agent-browser:
...
byCLI's current behavior and agent-browser's `select_option` behavior.
...
```bash
bycli browser click --role button --name "Submit"
bycli browser fill --label "Email" "me@example.com"
bycli browser get text --testid invoice-total
```
...
```bash
bycli browser click <target>
bycli browser dblclick <target>
bycli browser hover <target>
bycli browser focus <target>
bycli browser check <target>
bycli browser uncheck <target>
bycli browser upload <target> <file...>
bycli browser drag <source> <target>
bycli browser wait download [pattern]
```
...
- current byCLI baseline: often fails because click is DOM `el.click()`;
...
whether byCLI is approaching agent-browser reliability.

Execution process:

- Phase 0 completion: @bycli-质量官 runs Mercury, Brex, and Linear when access
  is available.
- On each form page, collect `bycli browser state --compare-sources` so the AX
...
- `skills/bycli-browser/SKILL.md`: recommend snapshot/click/snapshot/click for
...
- Comparison guide: byCLI's goal is not "Playwright in CLI form"; it is an
