...
byCLI's generic `browser click` currently calls DOM `el.click()` first. That
only dispatches a click event, so byCLI can return success while the dropdown
...
byCLI usually reaches Chrome through the extension, so command availability
must be explicit.
...
- byCLI verifies the selected text/value changed.
...
- update `skills/bycli-browser/SKILL.md`,
...
|---|---|---|
| CDP click primary | Event order changes from synthetic click to real mouse sequence. | This is desired for dropdowns. PR 1 must run full adapter tests; all new failures must be listed and fixed before merge. Keep `BYCLI_BROWSER_CLICK=js` as emergency plan B for one release, not as a substitute for fixing tests. |
...
- After Phase 0, @bycli-质量官 runs Mercury, Brex, and Linear manually when
...
- For each site, also run `bycli browser state --compare-sources` on the form
