...
cli({
  ...
  example: 'bycli mysite search <query> -f yaml',
...
Adapters may also declare `example` to override the canonical invocation shown in agent-facing help. Prefer YAML examples, e.g. `bycli mysite search <query> -f yaml`.
...
For most search/read/detail commands, the main subject should be positional (`bycli mysite search "rust"`, `bycli mysite article 123`) instead of a named flag such as `--query` or `--id`. Keep named flags for optional modifiers.
...
Use the `bycli-adapter-author` skill plus the `bycli browser *` primitives to scaffold and verify adapters end-to-end:
...
# Scaffold + verify
bycli browser init mysite/trending
bycli browser verify mysite/trending
```
