# Using byCLI with Android Chrome

byCLI can control Chrome on a connected Android device via **ADB port forwarding** and **CDPBridge** — no extra tools or custom builds required. The same adapters that run on desktop Chrome work identically on Android, reusing whatever cookies are already in the mobile browser.

---

## How It Works

Android Chrome supports [remote debugging via CDP](https://developer.chrome.com/docs/devtools/remote-debugging/). The device exposes a local Unix socket that ADB can forward to a TCP port on your machine. byCLI's `CDPBridge` then connects to that port exactly as it would to any other CDP endpoint.
...
- byCLI installed (`npm install -g bycli`)
...
### 4. Run any byCLI command

```bash
export BYCLI_CDP_ENDPOINT=http://localhost:9222
...
When multiple tabs are open, `CDPBridge` picks the best one automatically using a scoring algorithm (prefer `type=page`, real URLs over `about:blank`, etc.). To override this, set `BYCLI_CDP_TARGET` to a substring of the tab's title or URL:

```bash
BYCLI_CDP_TARGET="twitter" bycli twitter trending
```

You can also connect directly to a specific tab's WebSocket URL (from `/json`):

```bash
BYCLI_CDP_ENDPOINT=ws://localhost:9222/devtools/page/3941 bycli ...
```
...
Adapters that use the `cookie` strategy (most social/content sites) need you to be logged in on the Android device. The cookies are already in Android Chrome — byCLI reads them automatically over CDP.

To check whether an adapter requires login:

```bash
bycli zhihu hot --help
...
BYCLI_CDP_ENDPOINT=http://localhost:9222 bycli twitter trending
BYCLI_CDP_ENDPOINT=http://localhost:9223 bycli twitter trending
```
