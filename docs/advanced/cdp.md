# Connecting byCLI via CDP (Remote/Headless Servers)

If you cannot use the bycli Browser Bridge extension (e.g., in a remote headless server environment without a UI), byCLI provides an alternative: connecting directly to Chrome via **CDP (Chrome DevTools Protocol)**.
...
3. **Execution**: Run byCLI on your server.
...
Now switch to your **Remote Server** where byCLI is installed. 

Depending on the network tunnel method you chose in Phase 2, set the `BYCLI_CDP_ENDPOINT` environment variable and run your commands.

### If you used Method A (SSH Tunnel):

```bash
export BYCLI_CDP_ENDPOINT="http://localhost:9222"
bycli doctor                    # Verify connection
bycli bilibili hot --limit 5    # Test a command
```
...
export BYCLI_CDP_ENDPOINT="https://abcdef.ngrok.app"
bycli doctor                    # Verify connection
bycli bilibili hot --limit 5    # Test a command
```

> *Tip: If you provide a standard HTTP/HTTPS CDP endpoint, byCLI requests the `/json` target list and picks the most likely inspectable app/page target automatically. If multiple app targets exist, you can further narrow selection with `BYCLI_CDP_TARGET` (for example `antigravity` or `codex`).*
