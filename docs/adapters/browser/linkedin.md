...
| Command | Description |
|---------|-------------|
| `bycli linkedin connect` | Send a fail-closed connection request after verifying the exact profile |
| `bycli linkedin inbox` | List LinkedIn messaging inbox conversations and unread status |
| `bycli linkedin job-detail` | Read one LinkedIn job page with description, apply URL, workplace type, applicants, and company metadata |
| `bycli linkedin jobs-preferences` | Read visible LinkedIn Jobs preferences and alert settings without changing them |
| `bycli linkedin people-search` | Search standard LinkedIn for people by keyword (SSR DOM scrape). Each query counts toward LinkedIn's monthly Commercial Use Limit |
| `bycli linkedin profile-analytics` | Read visible profile dashboard counters such as profile views, post impressions, and search appearances |
| `bycli linkedin profile-experience` | Read visible LinkedIn profile experience entries with titles, dates, locations, skills, media, and URLs |
| `bycli linkedin profile-read` | Read visible profile sections including headline, About, experience, education, services, and featured text |
| `bycli linkedin posts` | Export visible posts from a LinkedIn profile activity page with engagement metrics |
| `bycli linkedin post-analytics` | Summarize raw visible LinkedIn post counters without custom scoring or classification |
| `bycli linkedin profile-projects` | Read visible LinkedIn profile projects with descriptions, dates, skills, media, and URLs |
| `bycli linkedin safe-send` | Verify exact recipient/thread context before optionally sending a message |
| `bycli linkedin salesnav-inbox` | List Sales Navigator message conversations with API pagination |
| `bycli linkedin salesnav-message` | Validate or send a Sales Navigator InMail to an exact lead |
| `bycli linkedin salesnav-search` | Search Sales Navigator people leads by keyword |
| `bycli linkedin salesnav-thread` | Return Sales Navigator message history for a thread or lead |
| `bycli linkedin search` | Search LinkedIn jobs (Voyager API), with optional `--details` enrichment |
| `bycli linkedin sent-invitations` | List pending sent LinkedIn invitations |
| `bycli linkedin services-read` | Read a LinkedIn Services page, including overview, services, availability, pricing, and media metadata |
| `bycli linkedin thread-snapshot` | Load a LinkedIn messaging thread and return available context |
| `bycli linkedin timeline` | Read posts from your LinkedIn home feed |

## Usage Examples

```bash
# Quick start
bycli linkedin search --limit 5

# Search with filters
bycli linkedin search "site reliability engineer" --location "San Francisco Bay Area" --remote remote

# Enrich with full description and apply URL (slower; 1 page navigation per row)
bycli linkedin search "data scientist" --limit 3 --details

# Read Jobs preferences and a single job page
bycli linkedin jobs-preferences -f json
bycli linkedin job-detail https://www.linkedin.com/jobs/view/4412279099 -f json

# Read your home timeline
bycli linkedin timeline --limit 5

# Read profile and services data
bycli linkedin profile-read -f json
bycli linkedin profile-analytics -f json
bycli linkedin services-read -f json

# Export visible profile activity posts
bycli linkedin posts --limit 5 -f json

# Summarize raw visible post counters
bycli linkedin post-analytics --limit 5 -f json

# Read visible profile projects
bycli linkedin profile-projects -f json

# Read visible profile experience entries
bycli linkedin profile-experience -f json

# List recent inbox conversations, including unread status
bycli linkedin inbox --limit 20 -f json

# Search for people by keyword (consumes 1 CUL search query)
bycli linkedin people-search "site reliability engineer berlin" --limit 5

# Verify a profile before sending a connection request; add --send to actually send
bycli linkedin connect https://www.linkedin.com/in/example/ --expected-name "Jane Doe" --note "quick note" --send

# Snapshot a thread, then safe-send only if exact recipient/thread context still matches
bycli linkedin thread-snapshot --thread-url https://www.linkedin.com/messaging/thread/abc/ -f json
bycli linkedin safe-send --thread-url https://www.linkedin.com/messaging/thread/abc/ --expected-name "Jane Doe" --message "thanks" --send

# Search Sales Navigator leads and inspect Sales Navigator messages
bycli linkedin salesnav-search "quality manager food manufacturing" --limit 10 -f json
bycli linkedin salesnav-inbox --limit 20 -f json
bycli linkedin salesnav-thread "https://www.linkedin.com/sales/inbox/2-thread" -f json

# Dry-run a Sales Navigator InMail; add --send only after validating the row
bycli linkedin salesnav-message "urn:li:fs_salesProfile:(PROFILE,NAME_SEARCH,TOKEN)" --subject "Quick question" --body "Hello"

# Reconcile pending sent invitations
bycli linkedin sent-invitations -f json

# JSON output
bycli linkedin search -f json
bycli linkedin timeline -f json
```
