...
| Command | Description |
|---------|-------------|
| `bycli indeed search <query>` | Keyword job search on the US site |
| `bycli indeed job <jk>` (alias `detail`, `view`) | Read the full job posting by `jk` (job key) |

## Usage Examples

```bash
# Quick start — first run will route through the browser session
bycli indeed search "rust developer" --limit 10

# Narrow to recent jobs in a location, sorted by date
bycli indeed search "site reliability engineer" \
  --location "Remote" --fromage 7 --sort date

# Read a job posting using the `id` surfaced by `search`
bycli indeed job dccc07ac5a6a3683

# JSON output
bycli indeed search "data engineer" -f json
```
...
identifier. Pipe it into `bycli indeed job <jk>` to drill into the full
posting (per the
