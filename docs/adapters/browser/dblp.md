...
| Command | Description |
|---------|-------------|
| `bycli dblp search <query>` | Free-text search across titles, authors, and venues |
| `bycli dblp author <name>` | List one author's recent publications (newest first) by name or `--pid` |
| `bycli dblp paper <key>` | Fetch a single record's full metadata by canonical dblp key |
| `bycli dblp venue <query>` | Search dblp's venue (conference / journal) registry |

## Usage Examples

```bash
# Free-text search (matches title / author / venue)
bycli dblp search "attention is all you need" --limit 5

# Author search
bycli dblp search "Yoshua Bengio" --limit 20

# Newest publications by a specific author (resolves the dblp PID for you)
bycli dblp author "Yoshua Bengio" --limit 20

# Same call, but skip name resolution by supplying the PID directly
bycli dblp author --pid 56/953 --limit 20

# Venue search
bycli dblp search "ICLR 2024" --limit 30

# Single-record detail (round-trip from search.key)
bycli dblp paper conf/nips/VaswaniSPUJGKP17

# arXiv mirror records use the journals/corr/abs-* form
bycli dblp paper journals/corr/abs-2509-05821

# Resolve a venue acronym to dblp's canonical venue page
bycli dblp venue ICLR --limit 5

# Browse venues that match a topic keyword
bycli dblp venue "neural networks" --limit 10

# JSON output
bycli dblp search "graph neural network" -f json
```
