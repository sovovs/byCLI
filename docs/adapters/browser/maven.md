...
| Command | Description |
|---------|-------------|
| `bycli maven search <query>` | Search Maven Central by keyword (artifact name, groupId, tag) |
| `bycli maven artifact <coordinate>` | Version history for `groupId:artifactId[:version]` |

## Usage Examples

```bash
# Free-text search
bycli maven search jackson --limit 10
bycli maven search "ai.koog" --limit 5

# Version history for a specific artifact (use `coordinate` from search rows)
bycli maven artifact com.fasterxml.jackson.core:jackson-databind --limit 10
bycli maven artifact com.google.guava:guava --limit 5

# Pin to a specific version
bycli maven artifact com.google.guava:guava:33.0.0-jre

# JSON output
bycli maven search jackson -f json
```
