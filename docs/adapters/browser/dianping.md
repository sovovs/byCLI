...
| Command | Description |
|---------|-------------|
| `bycli dianping search "<keyword>" --city <name-or-id> --limit <n>` | Search shops/restaurants on `www.dianping.com` |
| `bycli dianping shop <shop_id>` | Read shop detail by shop id (alias: `detail`) |

## Usage Examples

```bash
# Search "火锅" in 北京 (top 5 results, table view)
bycli dianping search 火锅 --city beijing --limit 5

# JSON output
bycli dianping search 火锅 --city 北京 --limit 5 -f json

# Search using a numeric cityId (path segment in dianping URLs)
bycli dianping search 火锅 --city 2

# Search using the cookie's currently-selected city (omit --city)
bycli dianping search 火锅

# Read a shop detail by id
bycli dianping shop GxJZ4urc9TnKE3kY

# `detail` alias works the same way
bycli dianping detail GxJZ4urc9TnKE3kY -f json

# Pass a full /shop/<id> URL — the id segment is auto-extracted
bycli dianping shop https://www.dianping.com/shop/GxJZ4urc9TnKE3kY
```
