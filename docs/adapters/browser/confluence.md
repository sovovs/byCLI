...
| Command | Description |
|---------|-------------|
| `bycli confluence page <PAGE_ID>` | Page detail with storage and Markdown body |
| `bycli confluence search <CQL>` | Search content with CQL |
| `bycli confluence create` | Create a page from Markdown or storage XHTML |
| `bycli confluence update <PAGE_ID>` | Update a page body from Markdown or storage XHTML |
...
```bash
# Read a page
bycli confluence page 123456 -f json

# Search pages in a space
bycli confluence search "type = page and title ~ \"RCA\"" --space ENG --limit 20 -f json

# Create a page from Markdown
bycli confluence create --space 987654 --title "PROJ-123 RCA" --file rca.md --execute

# Update an existing page
bycli confluence update 123456 --file rca.md --version-message "Sync from Jira PROJ-123" --execute
```
...
```bash
bycli confluence create --space 987654 --title "Runbook" --file runbook.md --execute
```

To provide Confluence storage XHTML directly:

```bash
bycli confluence update 123456 --file page.storage.html --representation storage --execute
```
