...
| Command | Description |
|---------|-------------|
| `bycli lichess user <username>` | Lichess player profile (rating, perfs, win/loss counts) |
| `bycli lichess top <perf>` | Top-N leaderboard for a perf type (bullet/blitz/rapid/classical/...) |

## Usage Examples

```bash
# Player profile
bycli lichess user DrNykterstein
bycli lichess user penguingm1

# Top-N leaderboards (username round-trips into `lichess user`)
bycli lichess top blitz
bycli lichess top bullet --limit 50
bycli lichess top rapid --limit 25
bycli lichess top chess960
```
