...
| Command | Description |
|---------|-------------|
| `bycli reuters search` | Search Reuters articles (`articles-by-search-v2` API) |
| `bycli reuters article-detail` | Fetch full article body + metadata for a Reuters URL |

## Usage Examples

```bash
# Search the latest Reuters articles
bycli reuters search "tariff" --limit 10

# Round-trip from search → detail using the `url` column
bycli reuters article-detail "https://www.reuters.com/world/..."

# JSON output
bycli reuters search "tariff" -f json
```
