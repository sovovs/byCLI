...
| Command | Description |
|---------|-------------|
| `bycli suno status` | Show login state, plan, credit breakdown, captcha readiness |
| `bycli suno list` | List recent clips in your library (id, title, status, created_at, link) |
| `bycli suno generate [prompt]` | Generate a song (Simple or Custom mode) and download clips locally |
| `bycli suno download <clip>` | Download an existing clip by UUID or `/song/<id>` URL |
...
bycli suno list --limit 20

# Simple mode — Suno picks lyrics, tags, and title
bycli suno generate "lo-fi study beat, 80 bpm, vinyl crackle" --instrumental true
...
bycli suno generate "post-rock crescendo" --weirdness 0.74 --style-weight 0.57

# Generate but skip the download (you only want the Suno links + clip ids)
bycli suno generate "ambient drone" --sd true

# Download an existing clip in MP3 + metadata (default)
bycli suno download a1b2c3d4-1111-2222-3333-444444444444
