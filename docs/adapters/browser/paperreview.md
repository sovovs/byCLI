...
| Command | Description |
|---------|-------------|
| `bycli paperreview submit` | Submit a PDF to paperreview.ai for review |
| `bycli paperreview review` | Fetch a review by token |
| `bycli paperreview feedback` | Send feedback on a completed review |

## Usage Examples

```bash
# Validate a local PDF without uploading it
bycli paperreview submit ./paper.pdf --email you@example.com --venue RAL --dry-run true

# Request an upload slot but stop before the actual upload
bycli paperreview submit ./paper.pdf --email you@example.com --venue RAL --prepare-only true

# Submit a paper for review
bycli paperreview submit ./paper.pdf --email you@example.com --venue RAL -f json

# Check the review status or fetch the final review
bycli paperreview review tok_123 -f json

# Submit feedback on the review quality
bycli paperreview feedback tok_123 --helpfulness 4 --critical-error no --actionable-suggestions yes
```
