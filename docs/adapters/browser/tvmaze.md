...
| Command | Description |
|---------|-------------|
| `bycli tvmaze search <query>` | TVmaze TV show search by title (returns id, name, network, premiered/ended, rating) |
| `bycli tvmaze show <id>` | Single TV show detail (network, schedule, rating, IMDB/TheTVDB cross-refs) |

## Usage Examples

```bash
# Title search
bycli tvmaze search "breaking bad"
bycli tvmaze search succession --limit 5

# Show detail (id from search)
bycli tvmaze show 169
bycli tvmaze show 169 -f json
```
