...
| Command | Description |
|---------|-------------|
| `bycli npm search <query>` | Search the public npm registry by keyword |
| `bycli npm package <name>` | Single-package registry metadata (latest version, license, repo, maintainers) |
| `bycli npm downloads <name>` | Download stats for one package over a fixed period or `YYYY-MM-DD:YYYY-MM-DD` range |

## Usage Examples

```bash
# Search the registry
bycli npm search react --limit 10
bycli npm search "graphql client" --limit 20

# Inspect a single package (use `name` from search rows)
bycli npm package react
bycli npm package @vercel/og

# Download stats for a fixed period
bycli npm downloads react --period last-week
bycli npm downloads react --period last-month
bycli npm downloads react --period last-year

# Custom date range (max 365 days, npm API limit)
bycli npm downloads react --period 2025-01-01:2025-01-31

# JSON output
bycli npm package react -f json
```
