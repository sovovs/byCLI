...
| Command | Description |
|---------|-------------|
| `bycli geogebra eval "<cmd1>;<cmd2>;..."` | Execute one or more GeoGebra commands in a fresh automation page |
| `bycli geogebra add-point --name A --coords 1,2` | Create one point |
| `bycli geogebra add-line --points A,B --type segment` | Create a line, segment, or ray from existing points |
| `bycli geogebra add-circle --center A --radius 3` | Create a circle from an existing center |
| `bycli geogebra add-polygon --points A,B,C` | Create a polygon from existing points |
| `bycli geogebra triangle --size 4` | Draw an equilateral triangle |
| `bycli geogebra hexagon --size 3` | Draw a regular hexagon |
| `bycli geogebra list` | List current objects on the canvas |
| `bycli geogebra info --name A` | Inspect one object |

## Two Workflows

### 1. Fresh automation page

Use the site command directly when byCLI is allowed to open its own GeoGebra page.

```bash
bycli geogebra triangle --size 4
bycli geogebra eval "A=(0,0);B=(4,0);c=Circle(A,B);d=Circle(B,A);C=Intersect(c,d,1);Polygon(A,B,C)"
```

Important:

- Each `bycli geogebra ...` command runs in its own fresh browser session.
...
```bash
bycli browser bind --workspace bound:geogebra --domain www.geogebra.org
bycli browser --workspace bound:geogebra get url
bycli browser --workspace bound:geogebra eval "(() => {
  const cmds = [
    'OCLIA=(0,0)',
    'OCLIB=(4,0)',
...
- you do not want byCLI to navigate away or replace the user's page state
...
- Start with `bycli doctor` if Browser Bridge behavior looks stale.
- If the user wants the current visible tab, bind first and operate through `bycli browser --workspace bound:geogebra ...`.
- If a fresh page is acceptable, use `bycli geogebra eval ...` or `bycli geogebra triangle`.
