...
| Command | Description |
|---------|-------------|
| `bycli crates search <query>` | Search the public crates.io registry by keyword |
| `bycli crates crate <name>` | Single crate metadata (latest version, downloads, license, repo) |

## Usage Examples

```bash
# Free-text search (name / keywords / description)
bycli crates search tokio --limit 10
bycli crates search "async runtime" --limit 20

# Single-crate detail (name from search rows)
bycli crates crate serde
bycli crates crate tokio

# JSON output
bycli crates search tokio -f json
```
