...
| Command | Description |
|---------|-------------|
| `bycli oeis search <query>` | Search OEIS sequences by keyword or numeric pattern |
| `bycli oeis sequence <id>` | Full OEIS sequence detail by A-number |

## Usage Examples

```bash
# Keyword search
bycli oeis search fibonacci
bycli oeis search "prime gaps" --limit 5

# Numeric pattern search (find sequences matching a prefix)
bycli oeis search "1,1,2,3,5,8"            # Fibonacci
bycli oeis search "2,3,5,7,11,13,17"       # primes

# Sequence detail (A-number round-trips from search)
bycli oeis sequence A000045                  # Fibonacci
bycli oeis sequence A000040                  # Primes
bycli oeis sequence A000041                  # Partitions
```
