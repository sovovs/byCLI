...
| Command | Description |
|---------|-------------|
| `bycli pypi package <name>` | Single PyPI package metadata (latest version, summary, repo, license, release timeline) |
| `bycli pypi downloads <name>` | Download stats from pypistats.org (recent totals or daily history) |

## Usage Examples

```bash
# Single-package metadata
bycli pypi package requests
bycli pypi package fastapi

# Download stats — three rows (last-day / last-week / last-month) totals
bycli pypi downloads requests --period recent

# Daily history for ~180 days (pypistats default window)
bycli pypi downloads requests --period overall

# JSON output
bycli pypi package requests -f json
```
