...
| Command | Description |
|---------|-------------|
| `bycli wikidata search <query>` | Search Wikidata items by label / alias (returns Q-IDs) |
| `bycli wikidata entity <id>` | Full entity detail: label, description, aliases, claim & sitelink counts |

## Usage Examples

```bash
# Find Q-IDs by keyword
bycli wikidata search einstein
bycli wikidata search "san francisco" --limit 10
bycli wikidata search 哈尔滨 --language zh

# Entity detail (Q-IDs round-trip from search)
bycli wikidata entity Q937           # Albert Einstein
bycli wikidata entity Q90            # Paris
bycli wikidata entity Q937 --language zh
```
