...
| Command | Description |
|---------|-------------|
| `bycli nuget search <query>` | Search NuGet packages by keyword |
| `bycli nuget package <id>` | Full NuGet package version history (catalog entries) |

## Usage Examples

```bash
# Keyword search
bycli nuget search newtonsoft
bycli nuget search "asp.net core" --limit 10
bycli nuget search serilog --prerelease=true

# Full version history (id round-trips from search)
bycli nuget package Newtonsoft.Json
bycli nuget package Serilog
bycli nuget package Microsoft.Extensions.Logging
```
