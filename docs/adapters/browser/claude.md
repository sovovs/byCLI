...
| Command | Description |
|---------|-------------|
| `bycli claude ask <prompt>` | Send a prompt and get the response |
| `bycli claude send <prompt>` | Send a prompt without waiting for the response |
| `bycli claude new` | Start a new conversation |
| `bycli claude status` | Check login state and page availability |
| `bycli claude read` | Read the current conversation |
| `bycli claude history` | List recent conversations from `/recents` |
| `bycli claude detail <id>` | Open a conversation by ID and read its messages |

## Usage Examples

```bash
# Ask a question
bycli claude ask "explain quicksort in 3 sentences"

# Start a new chat before asking
bycli claude ask "hello" --new

# Pick the model (default: sonnet; opus is paid-tier)
bycli claude ask "quick summary" --model haiku

# Enable Adaptive thinking
bycli claude ask "prove that sqrt(2) is irrational" --think

# Attach a file (image / PDF / text, up to ~1 MB raw)
bycli claude ask "describe this image" --file ./photo.png

# Combine modes
bycli claude ask "what does this PDF cover?" --file ./paper.pdf --think --new

# Custom timeout (default: 120s)
bycli claude ask "write a long essay" --timeout 240

# JSON output
bycli claude ask "hello" -f json
```
