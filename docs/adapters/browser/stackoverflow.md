...
| Command | Description |
|---------|-------------|
| `bycli stackoverflow hot` | Hot questions |
| `bycli stackoverflow search <query>` | Search questions |
| `bycli stackoverflow bounties` | Questions with active bounties |
| `bycli stackoverflow unanswered` | Unanswered questions |
| `bycli stackoverflow read <id>` | Read a question with answers and comments |
| `bycli stackoverflow user <name>` | Find users by display name (highest reputation first) |
| `bycli stackoverflow tag <tag>` | List questions tagged with a given tag (most active first) |
| `bycli stackoverflow related <id>` | List questions related to a given question id |
...
```bash
# Hot questions
bycli stackoverflow hot --limit 10

# Search questions
bycli stackoverflow search "async await" --limit 20

# Active bounties
bycli stackoverflow bounties --limit 10

# Unanswered questions
bycli stackoverflow unanswered --limit 10

# Tagged questions (id feeds stackoverflow read)
bycli stackoverflow tag rust --limit 10

# Related questions for a given question id (rows feed stackoverflow read)
bycli stackoverflow related 11227809 --limit 10
bycli stackoverflow related 11227809 --sort votes --limit 5

# User profile search (returns userId/profile URL rows)
bycli stackoverflow user "Jon Skeet" --limit 5

# Read a question with answers and comments
bycli stackoverflow read 11227809
bycli stackoverflow read 11227809 --answers-limit 3 --comments-limit 5

# JSON output
bycli stackoverflow hot -f json
bycli stackoverflow read 11227809 -f json
```
...
- `tag` returns question rows with `id`, so agents can call
  `bycli stackoverflow read <id>` without parsing URLs. `user` returns
