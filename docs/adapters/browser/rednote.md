...
| Command | Description |
|---------|-------------|
| `bycli rednote search` | Search notes by keyword (returns title, author, likes, URL, author URL) |
| `bycli rednote note` | Read full note content (title, author, description, likes, collects, comments, tags) |
| `bycli rednote comments` | Read comments from a note (`--with-replies` for nested 楼中楼 replies) |
| `bycli rednote user` | Get public notes from a user profile |
| `bycli rednote feed` | Home feed (reads the hydrated Pinia store) |
| `bycli rednote notifications` | Notifications (`--type mentions|likes|connections`) |
| `bycli rednote download` | Download images and videos from a note |

## Usage Examples

```bash
# Search for notes
bycli rednote search travel --limit 10

# Read a note's full content (pass URL from search results to preserve xsec_token)
bycli rednote note "https://www.rednote.com/search_result/<id>?xsec_token=..."

# Read comments with nested replies (楼中楼)
bycli rednote comments "https://www.rednote.com/search_result/<id>?xsec_token=..." --with-replies --limit 20

# JSON output
bycli rednote search travel -f json

# User profile notes
bycli rednote user 5b21f6564eacab3b38f05c39 --limit 10

# Download
bycli rednote download "https://www.rednote.com/search_result/<id>?xsec_token=..."
```
