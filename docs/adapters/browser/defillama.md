...
| Command | Description |
|---------|-------------|
| `bycli defillama protocols` | Top DeFi protocols by current TVL (slug, name, category, TVL, mcap, change, chains) |
| `bycli defillama protocol <slug>` | Single protocol details (current TVL, mcap, chains, twitter, github, description) |

## Usage Examples

```bash
# Top 30 protocols by TVL (default)
bycli defillama protocols

# Top 100, JSON output
bycli defillama protocols --limit 100 -f json

# Single protocol details (slug from protocols rows)
bycli defillama protocol aave-v3
bycli defillama protocol lido

# Parent protocols (e.g. "aave") aggregate their children's chains
bycli defillama protocol aave
```
