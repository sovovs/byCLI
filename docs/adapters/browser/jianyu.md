...
| Command | Description |
|---------|-------------|
| `bycli jianyu search "<query>" --limit <n> [--since_days <n>]` | Search Jianyu bid notices and keep only accessible detail links |
| `bycli jianyu detail "<url>"` | Extract detail-page evidence blocks from a search URL |

## Usage Examples

```bash
# Search by keyword
bycli jianyu search "procurement" --limit 20 -f json

# Search another keyword with an explicit recency window
bycli jianyu search "substation" --limit 10 --since_days 30 -f json

# Extract structured detail evidence
bycli jianyu detail "https://www.jianyu360.cn/nologin/content/....html" -f json
```
