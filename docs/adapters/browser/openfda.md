...
| Command | Description |
|---------|-------------|
| `bycli openfda drug-label <query>` | Search FDA-approved drug labels by brand or generic name |
| `bycli openfda food-recall` | FDA food recall and enforcement actions, most recent first |

## Usage Examples

```bash
# Drug label search
bycli openfda drug-label aspirin
bycli openfda drug-label lisinopril --limit 3

# Recent food recalls (no filter — all recent)
bycli openfda food-recall --limit 5

# Filter to Class I (most serious) recalls
bycli openfda food-recall --classification "Class I"

# Free-text Lucene search
bycli openfda food-recall --query salmonella
bycli openfda food-recall --query listeria --status Ongoing
```
