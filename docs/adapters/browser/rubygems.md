...
| Command | Description |
|---------|-------------|
| `bycli rubygems search <query>` | Search RubyGems.org gems by keyword |
| `bycli rubygems gem <name>` | Single-gem metadata (version, downloads, license, links) |

## Usage Examples

```bash
# Search gems
bycli rubygems search rails --limit 10
bycli rubygems search redis --limit 5

# Single-gem metadata (use `gem` from search rows)
bycli rubygems gem rails
bycli rubygems gem sidekiq

# JSON output
bycli rubygems search rails -f json
bycli rubygems gem rails -f json
```
