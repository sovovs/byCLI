...
| Command | Description |
|---------|-------------|
| `bycli openreview search <query>` | Full-text search across all OpenReview papers |
| `bycli openreview venue <venue>` | List papers at a venue (e.g. `"ICLR 2024 oral"` or full invitation id) |
| `bycli openreview author <profile>` | List submissions by an author profile id (e.g. `"~Yoshua_Bengio1"`), newest first |
| `bycli openreview paper <id>` | Show full metadata (incl. abstract) for a single paper |
| `bycli openreview reviews <forum>` | Show paper + threaded reviews/decisions/comments |

## Usage Examples

```bash
# Full-text search
bycli openreview search "diffusion model" --limit 10

# Browse a venue by display name (matched against content.venue)
bycli openreview venue "ICLR 2024 oral" --limit 20

# Browse a venue by full invitation id (use this when display names overlap)
bycli openreview venue "ICLR.cc/2025/Conference/-/Submission" --limit 50 --offset 0

# Every submission by an author profile id (find it on the author's openreview.net profile URL)
bycli openreview author "~Yoshua_Bengio1" --limit 20

# Single-paper detail (full abstract)
bycli openreview paper KS8mIvetg2

# Full review thread — paper + reviews + decision + author rebuttal
bycli openreview reviews KS8mIvetg2 --max-length 4000

# JSON output
bycli openreview search "LLM" -f json
```
