...
| Command | Description |
|---------|-------------|
| `bycli chatgpt ask <prompt>` | Send a prompt and wait for the visible response |
| `bycli chatgpt send <prompt>` | Send a prompt without waiting |
| `bycli chatgpt read` | Read the current conversation |
| `bycli chatgpt history` | List visible conversation history links from the sidebar |
| `bycli chatgpt detail <id-or-url>` | Open a conversation by `/c/<id>` and read it |
| `bycli chatgpt new` | Start a new conversation |
| `bycli chatgpt status` | Check page and login state |
| `bycli chatgpt image <prompt>` | Generate images in ChatGPT web and optionally save them locally |

## Usage Examples

```bash
# Ask and wait for the answer
bycli chatgpt ask "Summarize the tradeoffs of browser session reuse"

# Continue the same ChatGPT tab but do not wait for the answer
bycli chatgpt send "Now turn that into a checklist"

# Read the current conversation
bycli chatgpt read --markdown true

# List recent visible conversations and read one by id or URL
bycli chatgpt history --limit 10
bycli chatgpt detail "https://chatgpt.com/c/<conversation-id>"

# Start a fresh chat
bycli chatgpt new

# Generate an image and save it to the default directory
bycli chatgpt image "a cyberpunk city at night"

# Upload a local image, ask ChatGPT to edit it, and save the result
bycli chatgpt image "make the background blue" --image ./cat.png

# Upload multiple local images for a combined edit
bycli chatgpt image "combine these into a poster" --image ./cat.png,./logo.png

# Save to a custom output directory
bycli chatgpt image "a robot sketching on paper" --op ~/Downloads/chatgpt-images

# Only generate in ChatGPT and print the conversation link
bycli chatgpt image "a tiny watercolor fox" --sd true
```
