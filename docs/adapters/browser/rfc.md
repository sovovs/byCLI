...
| Command | Description |
|---------|-------------|
| `bycli rfc rfc <number>` | Single IETF RFC metadata (title, abstract, working group, authors, std level) |

## Usage Examples

```bash
# Modern RFCs
bycli rfc rfc 9000     # QUIC
bycli rfc rfc 9110     # HTTP semantics

# Classic RFCs
bycli rfc rfc 791      # IP
bycli rfc rfc 2616     # HTTP/1.1 (now obsoleted by 9110)

# Courtesy: "rfcN" prefix accepted
bycli rfc rfc rfc9000
```
