...
| Command | Description |
|---------|-------------|
| `bycli mdn search <query>` | Search MDN Web Docs by keyword |

## Usage Examples

```bash
# Web platform feature search
bycli mdn search fetch --limit 10
bycli mdn search flexbox --limit 5

# JS reference lookups
bycli mdn search "Array.prototype.map"

# Localized search (default: en-US)
bycli mdn search fetch --locale ja --limit 5
bycli mdn search fetch --locale zh-CN --limit 5

# JSON output
bycli mdn search fetch -f json
```
