...
| Command | Description |
|---------|-------------|
| `bycli rest-countries country <name>` | Look up countries by common / official name |
| `bycli rest-countries region <region>` | List every country in a region |

## Usage Examples

```bash
# Name search (substring, returns 1 or many)
bycli rest-countries country japan
bycli rest-countries country "united kingdom"
bycli rest-countries country guinea          # matches Guinea, Guinea-Bissau, etc.

# Region listing (sorted by population desc)
bycli rest-countries region europe
bycli rest-countries region asia --limit 20
bycli rest-countries region oceania
```
