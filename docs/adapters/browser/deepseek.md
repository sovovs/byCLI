...
| Command | Description |
|---------|-------------|
| `bycli deepseek ask <prompt>` | Send a prompt and get the response |
| `bycli deepseek new` | Start a new conversation |
| `bycli deepseek status` | Check login state and page availability |
| `bycli deepseek read` | Read the current conversation |
| `bycli deepseek history` | List conversation history from sidebar |
| `bycli deepseek detail <id>` | Read a specific conversation by ID or URL |
| `bycli deepseek send <id> <prompt>` | Send a prompt to a specific conversation without waiting for a response |

## Usage Examples

```bash
# Ask a question
bycli deepseek ask "explain quicksort in 3 sentences"

# Start a new chat before asking
bycli deepseek ask "hello" --new

# Use Expert model instead of Instant
bycli deepseek ask "prove that sqrt(2) is irrational" --model expert

# Use Vision model with an image
bycli deepseek ask "describe this image" --model vision --file ./image.png

# Enable DeepThink mode
bycli deepseek ask "prove that sqrt(2) is irrational" --think

# Enable web search
bycli deepseek ask "latest news about AI" --search

# Attach a file
bycli deepseek ask "summarize this document" --file ./report.pdf

# Combine modes
bycli deepseek ask "what happened today?" --model expert --think --search --new

# Custom timeout (default: 120s)
bycli deepseek ask "write a long essay" --timeout 180

# JSON output
bycli deepseek ask "hello" -f json

# Check login status
bycli deepseek status
...
bycli deepseek history --limit 10

# Read a specific conversation by UUID or /a/chat/s/<id> URL
bycli deepseek detail 749e6bbd-6a45-4440-beaa-ae5238bf06d8

# Send to a specific existing conversation
bycli deepseek send 749e6bbd-6a45-4440-beaa-ae5238bf06d8 "continue from the last answer"
```
