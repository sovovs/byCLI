...
| Command | Description |
|---------|-------------|
| `bycli flathub search <query>` | Search Flathub apps by keyword |
| `bycli flathub app <appId>` | Full Flathub appstream metadata for an app id |

## Usage Examples

```bash
# Keyword search
bycli flathub search firefox
bycli flathub search "image editor" --limit 10
bycli flathub search blender

# App detail (appId round-trips from search)
bycli flathub app org.mozilla.firefox
bycli flathub app org.gnome.Calculator
bycli flathub app org.blender.Blender
```
