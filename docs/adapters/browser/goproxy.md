...
| Command | Description |
|---------|-------------|
| `bycli goproxy module <path>` | Latest version + VCS origin metadata for a Go module |
| `bycli goproxy versions <path>` | Published version tags for a Go module (newest first) |

## Usage Examples

```bash
# Latest released version of a module
bycli goproxy module github.com/gin-gonic/gin
bycli goproxy module golang.org/x/net

# Every published tag, semver-sorted (descending)
bycli goproxy versions github.com/gin-gonic/gin

# Larger window
bycli goproxy versions github.com/spf13/cobra --limit 100

# Include publish times (one extra request per row)
bycli goproxy versions golang.org/x/net --limit 10 --with-time
```
