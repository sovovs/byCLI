...
`weixin get-public-account-info` searches the logged-in WeChat Official Accounts backend and returns public accounts plus their `fakeid`. Use that `fakeid` with `articles` or `save-articles`.
...
| `bycli weixin get-public-account-info` | Search backend public accounts and obtain `fakeid` values |
...
`get-public-account-info` and `articles` are intentionally separate: inspect all account matches, choose the correct `fakeid`, and then list its articles before saving them. A similar account name is never selected automatically.
...
| Command | Arguments |
|---------|-----------|
| `get-public-account-info` | required positional `<query>`; `--limit <positive integer>` (default `10`); `--auth-source browser\|env` (default `browser`) |
...
`get-public-account-info` columns are `nickname`, `fakeid`, and `alias`. Missing aliases are `null`.
