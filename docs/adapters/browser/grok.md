...
|---------|-------------|--------|
| `bycli grok status` | Page availability, login state, current model and session | read |
| `bycli grok history` | List recent conversations from the sidebar (requires login) | read |
| `bycli grok read` | Read messages in the current conversation | read |
| `bycli grok detail <id>` | Open a conversation by ID and read its messages | read |
| `bycli grok ask <prompt>` | Send a prompt and wait for the assistant reply | write |
| `bycli grok send <prompt>` | Fire-and-forget: send a prompt without waiting | write |
| `bycli grok new` | Start a fresh conversation | write |
| `bycli grok image <prompt>` | Generate images via Grok and return their URLs | write |
...
bycli grok history --limit 10

# Read the active conversation as markdown
bycli grok read --markdown true

# Read a specific historical conversation by ID (or full URL)
bycli grok detail 7c4197f2-10a1-4ebb-a84a-fea89f4f1d06
bycli grok detail https://grok.com/c/7c4197f2-10a1-4ebb-a84a-fea89f4f1d06 --markdown true

# Ask a question and wait for the reply
bycli grok ask "Explain quantum computing in simple terms"

# Ask in a brand-new chat
bycli grok ask "Hello" --new true

# Fire-and-forget (don't wait for the reply)
bycli grok send "continue the previous answer"

# Start a new conversation
bycli grok new

# Generate an image
bycli grok image "a cyberpunk mechanical owl, neon purple and blue" --new true
```
...
- DOM or product changes on Grok can break composer detection — `bycli grok status` is the quickest sanity check.
