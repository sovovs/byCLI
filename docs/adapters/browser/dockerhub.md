...
| Command | Description |
|---------|-------------|
| `bycli dockerhub search <query>` | Search Docker Hub repositories by keyword |
| `bycli dockerhub image <name>` | Repository metadata (stars, pulls, last updated, status) |

## Usage Examples

```bash
# Search repositories
bycli dockerhub search nginx --limit 10
bycli dockerhub search "bitnami redis" --limit 5

# Single repository metadata (use `image` from search rows)
bycli dockerhub image nginx              # implicit `library/nginx`
bycli dockerhub image library/nginx
bycli dockerhub image bitnami/redis

# JSON output
bycli dockerhub search nginx -f json
bycli dockerhub image nginx -f json
```
