...
|---------|-------------|------|
| `bycli 1point3acres hot` | Today's hot threads (by heat) | Public |
| `bycli 1point3acres latest` | Newest threads (by post time, desc) | Public |
| `bycli 1point3acres digest` | Editor-picked / featured threads | Public |
| `bycli 1point3acres forums` | List all forums (fid + name) | Public |
| `bycli 1point3acres forum <fid>` | List threads in a specific forum | Public |
| `bycli 1point3acres thread <tid>` | Thread detail + replies | Public |
| `bycli 1point3acres user <who>` | User profile (group / points / 大米 / posts) | Public |
| `bycli 1point3acres search <query>` | Full-text search | Cookie |
| `bycli 1point3acres notifications` | Site notifications (replies / mentions / reviews) | Cookie |

## Usage Examples

```bash
# Today's hot threads
bycli 1point3acres hot --limit 10

# Newest posts in the overseas-job-referral forum (fid=198)
bycli 1point3acres forum 198 --limit 20

# Read a thread (tid comes from any listing's `tid` column)
bycli 1point3acres thread 1158360 --limit 10

# Lookup a user (numeric → uid, otherwise username)
bycli 1point3acres user 12345
bycli 1point3acres user some-username

# Filter the forum list by keyword
bycli 1point3acres forums --filter 面经

# Search the site (requires login)
bycli 1point3acres search "OPT extension" --limit 10

# Notifications (requires login)
bycli 1point3acres notifications --kind mypost --limit 20
```

## Common Forum IDs

`145` (海外面经) · `198` (海外职位内推) · `27` (研究生申请) · `28` (博士申请) · `82` (NIW / EB-1A 移民).
Use `bycli 1point3acres forums` to see the full list.
