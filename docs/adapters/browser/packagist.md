...
| Command | Description |
|---------|-------------|
| `bycli packagist search <query>` | Search Packagist (PHP / Composer) packages by keyword |
| `bycli packagist package <name>` | Single-package metadata (version, downloads, license, repo, GitHub stars) |

## Usage Examples

```bash
# Search packages
bycli packagist search symfony --limit 10
bycli packagist search "laravel http" --limit 5

# Single-package metadata (use `package` from search rows; vendor/package required)
bycli packagist package symfony/console
bycli packagist package laravel/framework
bycli packagist package monolog/monolog

# JSON output
bycli packagist search symfony -f json
bycli packagist package symfony/console -f json
```
