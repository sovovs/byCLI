...
| Command | Description |
|---------|-------------|
| `bycli band bands` | List all Bands you belong to |
| `bycli band posts <band_no>` | List posts from a Band |
| `bycli band post <band_no> <post_no>` | Export full post content including nested comments |
| `bycli band mentions` | Show notifications where you were @mentioned |
...
bycli band posts 12345678 --limit 10

# Export a post with comments
bycli band post 12345678 987654321

# Export post body only (skip comments)
bycli band post 12345678 987654321 --comments false

# Export post and download attached photos
bycli band post 12345678 987654321 --output ./band-photos

# Show recent @mention notifications
bycli band mentions --limit 20

# Show only unread mentions
bycli band mentions --unread true

# Show all notification types
bycli band mentions --filter all
```
