...
|---------|------|-------------|
| `bycli ctrip search` | Public | Suggest cities, scenic spots, railway stations and landmarks |
| `bycli ctrip hotel-suggest` | Public | Suggest cities, business areas and individual hotels |
| `bycli ctrip hotel-search` | Browser (cookie) | List hotels for a city + check-in/out date range |
| `bycli ctrip flight` | Browser (cookie) | One-way flight search by IATA route + departure date |

## Usage Examples

```bash
# Destination suggest
bycli ctrip search 苏州 --limit 10

# Hotel-context suggest (cities / business areas / hotels)
bycli ctrip hotel-suggest 陆家嘴 --limit 5

# Hotel listing (city ID from `search` / `hotel-suggest`)
bycli ctrip hotel-search 2 --checkin 2026-05-20 --checkout 2026-05-21 --limit 10

# One-way flight search
bycli ctrip flight BJS SHA --date 2026-05-20 --limit 20

# JSON output
bycli ctrip search 上海 -f json
```
