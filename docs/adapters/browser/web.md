...
| Command | Description |
|---------|-------------|
| `bycli web read --url <url>` | Fetch any web page and export as Markdown |

## Usage Examples

```bash
# Read a web page and save as Markdown
bycli web read --url https://example.com/article

# Custom output directory
bycli web read --url https://example.com/article --output ./my-articles

# Skip image download
bycli web read --url https://example.com/article --download-images false

# JSON output
bycli web read --url https://example.com/article -f json
...
Cross-origin iframes are listed in diagnostics but not merged. If diagnostics reveal that the page data comes from an API endpoint, prefer a dedicated adapter or `bycli browser network --detail <key>` for structured data instead of forcing table-like data into Markdown.
