...
| Command | Description |
|---------|-------------|
| `bycli osv vulnerability <id>` | Single OSV.dev vulnerability detail (severity, affected packages, CVE/GHSA aliases) |
| `bycli osv query <package> --ecosystem <eco>` | Vulnerabilities affecting a package (optionally pinned to a version) |

## Usage Examples

```bash
# Specific advisory by GHSA id
bycli osv vulnerability GHSA-29mw-wpgm-hmr9

# Same advisory by CVE alias
bycli osv vulnerability CVE-2020-28500

# All known npm-lodash vulns (newest first)
bycli osv query lodash --ecosystem npm

# Vulns affecting a pinned version
bycli osv query lodash --ecosystem npm --version 4.17.20

# Cross-ecosystem queries
bycli osv query django --ecosystem PyPI --limit 10
bycli osv query log4j-core --ecosystem Maven
```
