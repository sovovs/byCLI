...
| Command | Description |
|---------|-------------|
| `bycli gemini new` | Start a new Gemini web chat |
| `bycli gemini ask <prompt>` | Send a prompt and return only the assistant reply |
| `bycli gemini image <prompt>` | Generate images in Gemini and optionally save them locally |
| `bycli gemini deep-research <prompt>` | Start a Gemini Deep Research run and confirm it |
| `bycli gemini deep-research-result <query>` | Export Deep Research report URL from a Gemini conversation |
...
bycli gemini ask "Reply with exactly: HELLO"

# Ask in a new chat and wait longer
bycli gemini ask "Summarize this design in 3 bullets" --new true --timeout 90

# Generate an icon image with short flags
bycli gemini image "Generate a tiny cyan moon icon" --rt 1:1 --st icon

# Only generate in Gemini and print the page link without downloading files
bycli gemini image "A watercolor sunset over a lake" --sd true

# Save generated images to a custom directory
bycli gemini image "A flat illustration of a robot" --op ~/tmp/gemini-images
```
