...
| Command | Description |
|---------|-------------|
| `bycli coupang search` | Search Coupang products with logged-in browser session |
| `bycli coupang product` | Read full product detail (price, rating, seller, delivery) for a product ID |
| `bycli coupang add-to-cart` | Add a product to the logged-in account's shopping cart |

## Usage Examples

```bash
# List rocket-shipping mice (rank, product_id, title, price, rating, ...)
bycli coupang search "마우스" --filter rocket --limit 10

# Round-trip: pick a product_id from search and pull full detail
bycli coupang product 7654321

# Pass a full product URL instead of an ID
bycli coupang product --url https://www.coupang.com/vp/products/7654321

# JSON output (any subcommand)
bycli coupang product 7654321 -f json

# Add to cart (write — must be logged in)
bycli coupang add-to-cart 7654321
```
