...
Run an byCLI command from a remote machine (a CI runner, an agent server, a sandbox) while the **browser session stays on your local laptop**. The remote command sees `localhost:19825` like usual; behind the scenes, traffic is tunneled back to the daemon and Chrome on your machine.
...
Treat the daemon port the way you'd treat your unlocked desktop: never put it on a network you don't fully trust. Native "extension talks to remote daemon" support was [proposed in #636](https://github.com/sovovs/byCLI/pull/636) and deferred until daemon authentication exists.
...
The remote `bycli` process needs **no flags, no env vars, no extension changes** — it connects to its own loopback, which the tunnel forwards to your laptop.
...
- If the SSH session drops, the tunnel drops; the remote `bycli` simply fails to connect rather than reaching some stale endpoint.
...
3. On the remote server, run a second **frpc** that maps the relay's exposed port back to the remote's `localhost:19825` (a `stcp` visitor or a plain `tcp` client+visitor pair). This way `bycli` on the remote keeps talking to its own loopback.
...
`bycli doctor` from the remote should report the same extension version your local Chrome is running.

## Caveats

- **Local Chrome must be running** for the duration of the remote command. Closing Chrome closes the extension's WebSocket; remote `bycli` calls will fail with a "no daemon" error until Chrome is reopened.
