...
`bycli` follows Unix `sysexits.h` conventions so it integrates naturally with shell pipelines and CI scripts.
...
```bash
bycli spotify status || echo "exit $?"   # 69 if browser not running

bycli gh issue list 2>/dev/null
[ $? -eq 77 ] && bycli gh auth login      # auto-auth if not logged in
```
