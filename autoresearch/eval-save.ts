...
const USER_CLIS_DIR = join(homedir(), '.bycli', 'clis');
...
/** Run a command, using the local built entrypoint instead of global bycli for consistency */
function runCommand(cmd: string, timeout = 30000): string {
  // Use local build so tests always run against the current source
  const localCmd = cmd.replace(/^bycli /, `node dist/src/main.js `);
  try {
...
function runTask(task: SaveTask): TaskResult {
  ...
  try {
    ...
    const initOutput = runCommand(`bycli browser init ${site}/${command}`);
