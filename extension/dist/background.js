...
//#endregion
//#region src/url-redact.ts
var AUTH_PARAM_SEGMENTS = new Set([
	"token",
	"tokens",
	"jwt",
	"secret",
...
var JWT_VALUE_RE = /^ey[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{6,}\.[A-Za-z0-9_-]+$/;
var MASKED = "***";
...
function maskUrlAuthTokens(url) {
	...
	try {
		const u = new URL(url);
		let changed = false;
		for (const [k, v] of [...u.searchParams.entries()]) if (paramNameLooksAuth(k) || JWT_VALUE_RE.test(v)) {
...
//#endregion
//#region src/ui-capture.ts
var UI_BINDING_NAME = "__bycli_ui";
var ALLOWED_TYPES = [
	"click",
	"input",
	"submit",
...
var clampStr = (v, max) => typeof v === "string" && v.length ? v.slice(0, max) : void 0;
/**
...
function parseUiEvent(payload) {
	...
	if (type === "navigate") {
		...
		return {
			...
			selector: "",
			tag: "document",
			url: maskUrlAuthTokens(rawUrl).slice(0, 2048)
		};
	}
	...
	const ev = {
		...
		ts,
		selector,
		tag: clampStr(raw.tag, 24) ?? "unknown"
	};
	...
	if (vs && typeof vs.len === "number" && Number.isFinite(vs.len)) {
		...
		ev.valueShape = {
			len: Math.max(0, Math.min(1e5, Math.floor(vs.len))),
			kind: k
		};
	}
	return ev;
...
*/
var UI_LISTENER_SOURCE = `(function(){
...
var sessionToFrameUrl = /* @__PURE__ */ new Map();
var sessionToParent = /* @__PURE__ */ new Map();
var armedChildSessions = /* @__PURE__ */ new Map();
var MAX_CHILD_SESSIONS_PER_TAB = 50;
var tabCaptureGeneration = /* @__PURE__ */ new Map();
...
var CDP_WS_FRAME_CAPTURE_LIMIT = 256 * 1024;
var MAX_WS_FRAMES_PER_CONN = 500;
var networkCaptures = /* @__PURE__ */ new Map();
var CAPTURE_RESOURCE_TYPES = new Set([
	"XHR",
	"Fetch",
	"Document"
]);
...
var uiCaptures = /* @__PURE__ */ new Map();
	...
	return await new Promise((resolve) => {
		...
		chrome.downloads.search({
			...
			const eligible = recent.filter((item) => {
				...
				const itemStartedAt = typeof item.startTime === "string" ? Date.parse(item.startTime) : NaN;
			...
			for (const item of eligible) if (item.state === "in_progress" && matchesDownloadPattern(item, pattern)) {
				inProgressIds.add(item.id);
				inspectById(item.id);
			}
		}).catch((err) => {
...
async function armChildSession(tabId, sessionId) {
	...
	try {
		...
		if (uiCaptures.has(tabId) && !st.ui) {
			const ok = await sendSafe(child, "Runtime.enable", {}) && await sendSafe(child, "Page.enable", {}) && await sendSafe(child, "Runtime.addBinding", { name: "__bycli_ui" }) && await sendSafe(child, "Page.addScriptToEvaluateOnNewDocument", { source: UI_LISTENER_SOURCE });
...
async function readNetworkCapture(tabId, filter) {
	const state = networkCaptures.get(tabId);
	if (!state) return [];
	let entries = state.entries.filter((e) => e.kind === "cdp-websocket" || !isStaticAssetContentType(e.responseContentType));
	if (filter) {
...
var AmbiguousIframeTargetError = class extends Error {
...
function normalizeUrlForMatch(raw) {
	try {
		...
		return {
			href: `${u.origin}${pathname}${u.search}`,
			origin: u.origin,
			pathname
		};
	} catch {
...
function resolveTargetFrameSessions(tabId, targetFrameUrl) {
	...
	for (const sid of states.keys()) {
		...
		candidates.push({
			sid,
			norm: normalizeUrlForMatch(frameUrl)
		});
	}
	const exact = candidates.filter((c) => c.norm && c.norm.href === target.href);
	if (exact.length === 1) return {
		kind: "ok",
		sessionIds: collectDescendants(tabId, exact[0].sid)
	};
	if (exact.length > 1) return {
		kind: "ambiguous",
		candidates: exact.map((c) => c.sid)
	};
	...
	if (loose.length === 1) return {
		kind: "ok",
		sessionIds: collectDescendants(tabId, loose[0].sid)
	};
	if (loose.length > 1) return {
		kind: "ambiguous",
		candidates: loose.map((c) => c.sid)
	};
	const sameOrigin = candidates.filter((c) => c.norm && c.norm.origin === target.origin);
	if (sameOrigin.length === 1) return {
		kind: "ok",
		sessionIds: collectDescendants(tabId, sameOrigin[0].sid)
	};
	if (sameOrigin.length > 1) return {
		kind: "ambiguous",
		candidates: sameOrigin.map((c) => c.sid)
	};
...
function applyFrameFilter(tabId, items, filter) {
	...
	if (resolve.kind !== "ok") return {
		items: [],
		resolve
	};
	const allow = resolve.sessionIds;
	return {
		items: items.filter((it) => it.frameSessionId !== void 0 && allow.has(it.frameSessionId)),
		resolve
	};
}
...
var fetchGuards = /* @__PURE__ */ new Map();
var fetchListenerRegistered = false;
function ensureFetchListener() {
	...
	chrome.debugger.onEvent.addListener((source, method, params) => {
		...
		if (method === "Fetch.requestPaused") {
			...
			if (guard.allow(url)) chrome.debugger.sendCommand({ tabId }, "Fetch.continueRequest", { requestId }).catch(() => {});
			else {
...
function registerListeners() {
	...
	chrome.debugger.onEvent.addListener(async (source, method, params) => {
		if (method === "Target.attachedToTarget") {
			...
			if (typeof ap.targetInfo.url === "string") sessionToFrameUrl.set(childSessionId, maskUrlAuthTokens(ap.targetInfo.url));
		...
		if (method === "Target.targetInfoChanged") {
			...
			if (sid && ti?.type === "iframe" && typeof ti.url === "string" && ti.url) sessionToFrameUrl.set(sid, maskUrlAuthTokens(ti.url));
			return;
		}
			...
			if (bp?.name === "__bycli_ui") {
				...
				if (ui) {
					...
					if (ev) {
						...
						if (ui.events.length < 2e3) ui.events.push(ev);
						else ui.dropped++;
					}
				}
			}
		...
		if (method === "Network.webSocketCreated") {
			const wsEntry = getOrCreateNetworkCaptureEntry(tabId, reqKey(sessionId, String(eventParams?.requestId || "")), {
				url: maskUrlAuthTokens(eventParams?.url),
				method: "GET"
			}, "cdp-websocket");
...
//#endregion
//#region src/url-policy.ts
var ALLOWED_PROTOCOLS = new Set(["http:", "https:"]);
...
function parseRadixPart(part) {
	...
	} else if (!/^[0-9]+$/.test(part)) return null;
...
function ipv6InForbiddenRange(ip) {
	...
	if (g.slice(0, 5).every((x) => x === 0) && g[5] === 65535) return ipv4InForbiddenRange(`${g[6] >> 8 & 255}.${g[6] & 255}.${g[7] >> 8 & 255}.${g[7] & 255}`);
	const first = g[0];
...
function checkUrlSyntax(input, opts = {}) {
	...
	if (!ALLOWED_PROTOCOLS.has(parsed.protocol)) return {
		...
		detail: `protocol ${parsed.protocol} not allowed`
	};
	if (parsed.username !== "" || parsed.password !== "") return {
		ok: false,
		reason: "userinfo_present",
		detail: "userinfo (user:pass@) not allowed"
	};
	...
	if (ipLiteral !== null) {
		if (isForbiddenIp(ipLiteral)) return {
			ok: false,
			reason: "forbidden_ip",
			detail: `literal IP ${ipLiteral} is forbidden`
		};
		if (!opts.allowLiteralIp) return {
			ok: false,
			reason: "literal_ip_host",
			detail: `literal IP host ${ipLiteral} not allowed for navigation`
		};
		return {
			ok: true,
			url: parsed.toString(),
...
var EXTENSION_CAPABILITIES = ["focus-window-v1"];
var ws = null;
var reconnectTimer = null;
var reconnectAttempts = 0;
var CONTEXT_ID_KEY = "bycli_context_id_v1";
...
var leaseRevisions = /* @__PURE__ */ new Map();
var leaseIdleGenerations = /* @__PURE__ */ new Map();
var leaseOperationTails = /* @__PURE__ */ new Map();
var IDLE_TIMEOUT_DEFAULT = 3e4;
var IDLE_TIMEOUT_INTERACTIVE = 6e5;
var IDLE_TIMEOUT_NONE = -1;
var REGISTRY_KEY = "bycli_target_lease_registry_v2";
var LEASE_IDLE_ALARM_PREFIX = "bycli:lease-idle:";
var CONTAINER_TAB_GROUP_TITLE = {
	interactive: "byCLI Browser",
	automation: "byCLI Adapter"
};
var LEGACY_AUTOMATION_TAB_GROUP_TITLE = "byCLI";
...
function getSessionName(session) {
	...
	if (!raw) throw new CommandFailure("session_required", "Browser session is required.", "Pass a browser session name, e.g. bycli browser <session> <command>.");
	return raw;
}
...
function leaseKeyFromAlarmName(name) {
	...
	try {
		return decodeURIComponent(name.slice(17));
	} catch {
		return null;
	}
}
...
function withLeaseLock(leaseKey, fn) {
	const run = (leaseOperationTails.get(leaseKey) ?? Promise.resolve()).then(fn);
...
function scheduleIdleAlarmAt(leaseKey, deadline) {
	...
	try {
		if (deadline > 0) chrome.alarms?.create?.(alarmName, { when: deadline });
...
async function getOwnedContainerGroupId(role, windowId) {
	...
	if (container.groupId !== null) {
		try {
			if ((await chrome.tabGroups.get(container.groupId)).windowId === windowId) return container.groupId;
		} catch {}
...
async function discoverOwnedContainerFromTabGroup(role) {
	...
	for (const title of getOwnedContainerGroupTitles(role)) {
		...
		const selected = selectOwnedContainerDiscoveryCandidate((await Promise.all(groups.map(toOwnedContainerDiscoveryCandidate))).filter((candidate) => candidate !== null));
...
async function ensureOwnedContainerTabGroup(role, windowId, tabIds) {
	...
	const container = ownedContainers[role];
	const trackedGroupPromise = (container.groupPromise ?? Promise.resolve()).catch(() => void 0).then(() => ensureOwnedContainerTabGroupUnlocked(role, windowId, ids)).finally(() => {
...
async function findReusableOwnedContainerTab(windowId, groupId) {
	...
	try {
		return (await chrome.tabs.query({
			windowId,
			groupId
		})).find((tab) => tab.id !== void 0 && initialTabIsAvailable(tab.id) && isDebuggableUrl(tab.url))?.id;
	} catch {
		return;
	}
}
...
async function getAutomationWindow(leaseKey, initialUrl) {
	...
	if (existing) {
		if (!existing.owned) throw new CommandFailure("bound_window_operation_blocked", `Session "${existing.session}" is bound to a user tab and does not own an byCLI tab lease.`, "Use page commands on the bound tab, or unbind the session first.");
		try {
	...
	return (await ensureOwnedContainerWindow(getOwnedWindowRole(leaseKey), initialUrl, getWindowModeForOwnedPage(leaseKey, initialUrl && isSafeNavigationUrl(initialUrl) ? initialUrl : BLANK_PAGE))).windowId;
}
chrome.windows.onRemoved.addListener(async (windowId) => {
	...
	for (const [leaseKey, expected] of [...automationSessions.entries()]) if (expected.windowId === windowId) await withLeaseLock(leaseKey, async () => {
		const session = automationSessions.get(leaseKey);
...
chrome.tabs.onRemoved.addListener(async (tabId) => {
	evictTab(tabId);
	for (const [leaseKey, expected] of [...automationSessions.entries()]) if (expected.preferredTabId === tabId) await withLeaseLock(leaseKey, async () => {
		const session = automationSessions.get(leaseKey);
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
	...
	if (leaseKey) {
		const session = automationSessions.get(leaseKey);
		if (!session) return;
		await releaseLeaseIfIdleExpired(leaseKey, session, leaseIdleGenerations.get(leaseKey) ?? 0, Number.isFinite(alarm.scheduledTime) && alarm.scheduledTime > 0 ? alarm.scheduledTime : session.idleDeadlineAt, "idle alarm");
	}
});
...
async function handleCommand(cmd) {
	...
	try {
		switch (cmd.action) {
			...
			case "ui-capture-start": return await handleUiCaptureStart(cmd, leaseKey);
			case "ui-capture-read": return await handleUiCaptureRead(cmd, leaseKey);
...
async function resolveTabUnlocked(tabId, leaseKey, initialUrl, blankFirst = false) {
	...
	if (tabId !== void 0) try {
		...
		if (session && !session.owned) throw new CommandFailure(matchesSession ? "bound_tab_not_debuggable" : "bound_tab_mismatch", matchesSession ? `Bound tab for session "${session.session}" is not debuggable (${tab.url ?? "unknown URL"}).` : `Target tab is not the tab bound to session "${session.session}".`, "Run \"bycli browser bind\" again on a debuggable http(s) tab.");
		...
		} else if (!isDebuggableUrl(tab.url)) console.warn(`[bycli] Tab ${tabId} URL is not debuggable (${tab.url}), re-resolving`);
	} catch (err) {
		...
		if (existingSession && !existingSession.owned) {
			...
			throw new CommandFailure("bound_tab_gone", `Bound tab for session "${existingSession.session}" no longer exists.`, "Run \"bycli browser bind\" again, then retry the command.");
		}
	...
	if (existingSession && existingPreferredTabId !== null) {
		...
		try {
			...
			if (!session.owned) throw new CommandFailure("bound_tab_not_debuggable", `Bound tab for session "${session.session}" is not debuggable (${preferredTab.url ?? "unknown URL"}).`, "Switch the tab to an http(s) page or run \"bycli browser bind\" on another tab.");
		} catch (err) {
			if (err instanceof CommandFailure) throw err;
			await removeLeaseSession(leaseKey, session);
			if (!session.owned) throw new CommandFailure("bound_tab_gone", `Bound tab for session "${session.session}" no longer exists.`, "Run \"bycli browser bind\" again, then retry the command.");
			return createOwnedTabLeaseUnlocked(leaseKey, initialUrl, blankFirst);
		}
	}
	if (!existingSession || existingSession.owned && existingSession.preferredTabId === null) return createOwnedTabLeaseUnlocked(leaseKey, initialUrl, blankFirst);
...
async function resolveTabIdUnlocked(tabId, leaseKey, initialUrl) {
	return (await resolveTabUnlocked(tabId, leaseKey, initialUrl)).tabId;
}
...
async function listAutomationWebTabsUnlocked(leaseKey) {
	return (await listAutomationTabsUnlocked(leaseKey)).filter((tab) => isDebuggableUrl(tab.url));
}
...
async function handleNavigateUnlocked(cmd, leaseKey) {
	...
	const resolved = await resolveTabUnlocked(await resolveCommandTabId(cmd), leaseKey, cmd.url, true);
	...
	await new Promise((resolve) => {
		...
		blockTimer = setInterval(() => {
			if ((guard?.blocked.length ?? 0) > 0) finish();
		}, 100);
	...
	if ((guard?.blocked.length ?? 0) > 0) return {
		id: cmd.id,
		ok: false,
		errorCode: "navigation_blocked_by_policy",
	...
	if (navWindowMode === "foreground") await chrome.tabs.update(tabId, { active: true }).catch(() => {});
...
function focusLeaseChangedResult(id) {
	return {
		id,
		ok: false,
		error: "Current automation lease changed during focus; no further focus action was performed"
	};
}
async function handleFocusUnlocked(cmd, leaseKey) {
	...
	if (!focusLease) return {
		id: cmd.id,
		ok: false,
		error: "No owned tab is leased to the current automation session"
	};
	...
	if (tabId !== focusLease.preferredTabId) return {
		id: cmd.id,
		ok: false,
		error: "Page is not leased to the current automation session"
	};
	let tab;
	try {
		tab = await chrome.tabs.get(tabId);
		...
		return {
			id: cmd.id,
			ok: false,
			error: "Current automation session tab no longer exists"
		};
	}
	if (!isFocusLeaseCurrent(leaseKey, focusLease)) return focusLeaseChangedResult(cmd.id);
	if (!Number.isInteger(tab.windowId) || tab.windowId < 0 || tab.windowId !== focusLease.windowId) return {
		id: cmd.id,
		ok: false,
		error: "Current automation session tab is outside its owned window"
	};
	const updateWindow = chrome.windows.update;
	...
	if (!isFocusLeaseCurrent(leaseKey, focusLease)) return focusLeaseChangedResult(cmd.id);
	if (activatedTab.id !== tabId || activatedTab.windowId !== focusLease.windowId) return {
		id: cmd.id,
		ok: false,
		error: "Current automation session tab changed window during focus"
	};
...
async function handleTabNewUnlocked(cmd, leaseKey) {
	const currentSession = automationSessions.get(leaseKey);
	if (currentSession && !currentSession.owned) return boundTabMutationResult(cmd, currentSession);
...
async function handleTabCloseUnlocked(cmd, leaseKey) {
	...
	if (lockedSession && !lockedSession.owned) return boundTabMutationResult(cmd, lockedSession);
	if (cmd.index !== void 0) {
		const target = (await listAutomationWebTabsUnlocked(leaseKey))[cmd.index];
		if (!target?.id) return {
			id: cmd.id,
			ok: false,
		...
		if (automationSessions.get(leaseKey)?.preferredTabId === target.id) await releaseLeaseUnlocked(leaseKey, "tab close");
		else {
	...
	const tabId = await resolveTabIdUnlocked(await resolveCommandTabId(cmd), leaseKey);
	const closedPage = await resolveTargetId(tabId).catch(() => void 0);
	if (automationSessions.get(leaseKey)?.preferredTabId === tabId) await releaseLeaseUnlocked(leaseKey, "tab close");
	else {
...
async function handleTabs(cmd, leaseKey) {
	const session = automationSessions.get(leaseKey);
	if (session && !session.owned && cmd.op !== "list") return boundTabMutationResult(cmd, session);
	switch (cmd.op) {
		case "list": {
		...
		case "new": return withLeaseLock(leaseKey, () => handleTabNewUnlocked(cmd, leaseKey));
		case "close": return withLeaseLock(leaseKey, () => handleTabCloseUnlocked(cmd, leaseKey));
		case "select": return withLeaseLock(leaseKey, () => handleTabSelectUnlocked(cmd, leaseKey));
		case "focus": return withLeaseLock(leaseKey, () => handleFocusUnlocked(cmd, leaseKey));
		default: return {
			id: cmd.id,
			ok: false,
...
async function handleCdp(cmd, leaseKey) {
	...
	try {
		...
		const data = routeFrameId ? await sendCommandInFrameTarget(tabId, routeFrameId, cmd.cdpMethod, stripbyCliFrameRoutingParams(params, true), aggressive, 3e4, routeTargetUrl) : await chrome.debugger.sendCommand({ tabId }, cmd.cdpMethod, stripbyCliFrameRoutingParams(params, false));
		return pageScopedResult(cmd.id, tabId, data);
	} catch (err) {
		return {
			id: cmd.id,
...
async function releaseLeaseUnlocked(leaseKey, reason) {
	...
	if (session.owned) {
		...
		} else console.log(`[bycli] Released legacy owned window lease ${session.windowId} without closing container (session=${session.session}, surface=${session.surface}, ${reason})`);
...
async function handleBindUnlocked(cmd, leaseKey) {
	if (automationSessions.get(leaseKey)?.owned) await releaseLeaseUnlocked(leaseKey, "rebind");
