...
describe('background tab isolation', () => {
  ...
  it('returns the persisted profile contextId from popup status', async () => {
    const { chrome } = createChromeMock();
    await chrome.storage.local.set({ bycli_context_id_v1: 'abc123xy' });
    vi.stubGlobal('chrome', chrome);
  ...
  it('reconciles an owned container with no stored leases without closing it', async () => {
    ...
    await chrome.storage.local.set({
      bycli_target_lease_registry_v2: {
        version: 2,
  ...
  it('restores owned and borrowed leases from the registry', async () => {
    ...
    expect(chrome.alarms.create).toHaveBeenCalledWith(
      `bycli:lease-idle:${encodeURIComponent(adapterKey('twitter'))}`,
  ...
  it('marks a newly created owned automation window with an byCLI Adapter tab group', async () => {
  ...
  it('keeps Adapter windows foreground when the client requests background mode', async () => {
    const { chrome } = createChromeMock();
    vi.stubGlobal('chrome', chrome);

    const mod = await import('./background');
    const result = await mod.__test__.handleCommand({
      id: 'adapter-background-override',
      action: 'tabs',
      op: 'new',
      session: 'twitter',
      surface: 'adapter',
  ...
  it('discovers and reuses an existing byCLI Adapter group after service worker restart', async () => {
    const { chrome, tabs, groups } = createChromeMock();
    groups.push({
      id: 99,
      windowId: 1,
  ...
  it('discovers and reuses an existing byCLI Adapter group in another window before creating one', async () => {
    const { chrome, tabs, groups } = createChromeMock();
    tabs.push({
      id: 77,
      windowId: 7,
  ...
  it('prefers a focused byCLI Adapter group when multiple matching groups exist', async () => {
  ...
  it('prefers an byCLI Adapter group with a reusable debuggable tab when none are focused', async () => {
    const { chrome, tabs, groups, setLastFocusedWindowId } = createChromeMock();
  ...
  it('uses the lowest group id as the final byCLI Adapter group tiebreaker', async () => {
    const { chrome, tabs, groups, setLastFocusedWindowId } = createChromeMock();
    setLastFocusedWindowId(2);
    tabs.push({
      id: 77,
  ...
  it('discovers and reuses a legacy byCLI automation group before creating a duplicate', async () => {
    const { chrome, tabs, groups } = createChromeMock();
    tabs.push({
      id: 78,
      windowId: 8,
