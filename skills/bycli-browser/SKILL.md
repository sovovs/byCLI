---
name: bycli-browser
description: Use when an agent needs to drive a real Chrome window via bycli — inspect a page, fill forms, click through logged-in flows, or extract data ad-hoc. Covers the selector-first target contract, compound form fields, stale-ref handling, network capture, and the agent-native envelopes the CLI returns. Not for writing adapters — see bycli-adapter-author for that.
allowed-tools: Bash(bycli:*), Read, Edit, Write
---
...
This skill is for **driving a live browser** to accomplish an agent task. If you are building a reusable adapter under `~/.bycli/clis/<site>/` use `bycli-adapter-author` instead.
...
- Use a stable session name for any multi-command or human-paced browser workflow. Example: `bycli browser fb-yaya-warmup open https://example.com`, then reuse `bycli browser fb-yaya-warmup state`, `extract`, `click`, etc.
...
- `--window foreground|background` (or `BYCLI_WINDOW=foreground|background`) chooses whether byCLI creates/focuses a foreground browser window or uses a background browser window for owned sessions.
...
bycli browser gmail network
bycli browser gmail unbind
```

Binding never owns the user window and never closes the user tab. It fails closed if the tab is closed or becomes non-debuggable. Re-run `bycli browser <session> bind` when you switch to a different real tab.

Navigation is allowed on bound sessions because the session now represents explicit agent ownership of that tab. Tab mutation (`tab new`, `tab select`, `tab close`) is still blocked for bound sessions. Use an owned session when you want byCLI to manage tab lifecycle.

Bound sessions have no byCLI idle-close timer; the binding lasts until `unbind`, tab close, window close, or daemon restart.
...
2. **Prefer site adapters before raw browser driving.** If `bycli <site> <command>` already covers the task, use that adapter command first (`bycli facebook notifications`, `bycli reddit read`, etc.). Use `bycli browser ...` only for gaps, debugging, or one-off UI flows the adapter does not expose.
...
List entries look like `{key, method, status, url, ct, size, shape, body_truncated?}`. Detail envelope is `{key, url, method, status, ct, size, shape, body, body_truncated?, body_full_size?, body_truncation_reason}`. Cache lives in `~/.bycli/cache/browser-network/` so you can re-inspect without re-triggering the request.
...
bycli browser login state                          # fresh refs on the logged-in page
```

### Pick from a long dropdown

```bash
bycli browser form state                          # sidebar shows [12] <select name=country>
...
```bash
bycli browser mercury state                          # find category trigger ref
# If the trigger/option is not clear, use AX:
bycli browser mercury state --source ax              # look for combobox/button/listbox/option names
bycli browser mercury click 7                        # click category trigger
bycli browser mercury state --source ax              # fresh refs after the portal/listbox opens
bycli browser mercury click 12                       # click option
bycli browser mercury get text 7                     # verify visible selected label
```
...
| symptom | fix |
|---------|-----|
| `bycli doctor` red: "Browser not connected" | Start Chrome with `--remote-debugging-port=9222`, or install the extension from the [Chrome Web Store](https://chromewebstore.google.com/detail/bycli/ildkmabpimmkaediidaifkhjpohdnifk). |
...
| Network cache seems stale | Bump `--ttl` down, or let it expire. The cache lives at `~/.bycli/cache/browser-network/`. |

---

## See also

- `bycli-adapter-author` — turning what you just figured out into a reusable `~/.bycli/clis/<site>/<command>.js`.
- `bycli-autofix` — when an existing adapter breaks, this skill walks you through `--trace retain-on-failure` evidence and filing a fix.
