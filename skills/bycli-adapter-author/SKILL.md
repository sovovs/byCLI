---
name: bycli-adapter-author
description: Use when writing an byCLI adapter for a new site or adding a new command to an existing site. Guides end-to-end from first recon through field decoding, adapter coding, and verify. Replaces bycli-oneshot / bycli-explorer. For ad-hoc browser driving (no adapter), see bycli-browser instead; for a top-level orientation to bycli, see bycli-usage.
allowed-tools: Bash(bycli:*), Read, Edit, Write, Grep
---

# bycli-adapter-author

你是要给一个站点写 adapter 的 agent。这份 skill 目标：**从零到通过 `bycli browser verify` 的 30 分钟内闭环**。

全程用现有工具：`bycli browser *` / `bycli doctor` / `bycli browser init` / `bycli browser verify`。没有新命令。
...
START
  ...
  ▼
┌──────────────────────────┐
│ bycli doctor 通？      │── no ──→ 修桥接（doctor 输出里的提示）
└──────────────────────────┘
  │ yes
  ▼
┌────────────────────────────────────────────────────┐
...
└──────────────────────────┘
  ...
  ▼
┌──────────────────────────┐
│ bycli browser init      │  生成 ~/.bycli/clis/<site>/<name>.js 骨架
│ 复制最像的邻居 adapter    │
│ 改 name / URL / 映射三处  │
└──────────────────────────┘
  │
  ▼
┌──────────────────────────┐
│ bycli browser verify    │── 失败 ──→ autofix skill，用 --trace retain-on-failure 回对应步骤
└──────────────────────────┘
  │ 成功
  ▼
┌──────────────────────────┐
...
└──────────────────────────┘
  ...
  ▼
┌──────────────────────────┐
│ 回写 ~/.bycli/sites/   │  endpoints / field-map / notes / fixtures
└──────────────────────────┘
  │
  ▼
DONE
...
[ ] 2. 读站点记忆：
       [ ] ~/.bycli/sites/<site>/endpoints.json 存在？里面有想要的 endpoint？
...
[ ] 3. 侦察（site-recon.md）：
       [ ] **首选**：`bycli browser analyze <url>` 一步拿 pattern + 反爬 + 最近 adapter + next step
...
[ ] 9. 写 adapter（adapter-template.md）：
       ...
       [ ] 改 name / URL / 字段映射
[ ] 10. bycli browser verify <site>/<name>
        [ ] 首轮通过后立刻 `--write-fixture` 生成 `~/.bycli/sites/<site>/verify/<cmd>.json` 种子
        [ ] 手改种子：加 `patterns`（URL / 日期 / ID 格式）+ `notEmpty`（核心字段）+ 收紧 `rowCount`
        [ ] 再跑一次 `bycli browser verify <site>/<name>`，确认 ✓ matches fixture
...
[ ] 12. 回写站点记忆（**verify 通过 + 肉眼比对对得上之后**，schema 见 `references/site-memory.md`）：
        ...
        [ ] `verify/<cmd>.json`：**必填。** `bycli browser verify` 的期望值（args / rowCount / columns / types / patterns / notEmpty），Step 10 已经让你生成了，这里只是 checklist
        [ ] `fixtures/<cmd>-<YYYYMMDDHHMM>.json`：存一份该 endpoint 的完整响应样本（去掉 cookie / token / 用户私有字段再存），给后续字段对比 / 离线 replay 用
        [ ] 调试过程中如果在 repo / adapter 目录 dump 过临时文件（`.dbg-*.html` / `raw-*.json` / 等），**在 commit 前清干净**——这些本来就该落在 `~/.bycli/sites/<site>/fixtures/` 或 `/tmp/`
```
...
| `references/site-memory.md` | 总览：in-repo 种子 + 本地 `~/.bycli/sites/` 的两层结构 |
...
- adapter 只引 `@sovovs/bycli/registry` + `@sovovs/bycli/errors`，不用第三方
...
- 写私人 adapter 用 `~/.bycli/clis/<site>/<name>.js`（免 build）；要提 PR 才 copy 到 `clis/<site>/<name>.js`
- 站点记忆每轮回写：没记忆 → 用 skill → 产生记忆 → 下次变 5 分钟
- **调试过程中的原始 dump / 抓包 / HTML 样本只能落在 `~/.bycli/sites/<site>/fixtures/` 或 `/tmp/`。严禁在 repo 根目录、`clis/<site>/` 或当前工作目录留 `.dbg-*.html / raw-*.json / sample.*` 这类临时文件**（PR diff 会带上去，别人 review 时很烦）。
...
- 诊断类：`bycli doctor` → 看 `notes.md` → 搜 autofix skill
