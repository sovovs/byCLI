...
```
skills/bycli-adapter-author/references/site-memory/<site>.md
...
## Layer 2 — 本地工作目录（`~/.bycli/sites/<site>/`）
...
~/.bycli/sites/<site>/
  ...
  verify/                — `bycli browser verify` 期望值（值级校验锚点，每个 adapter 一份）
    <cmd>.json
...
每个 adapter 一份，`bycli browser verify <site>/<cmd>` 会自动读。**没有这份 = verify 只能证"能跑"，证不出数据对**——所以是必填产物。
...
调试过程里的临时 dump（`.dbg-*.html` / `raw-*.json` / `sample-*` / `trace-*.txt`）**只能**落在 `~/.bycli/sites/<site>/fixtures/` 或系统 `/tmp/`。PR diff 会把 repo 根目录和 `clis/<site>/` 下的文件一起带走——别人 review 时看到一堆调试副产物会很烦。

---

## 没有 site-memory 时

新站点没对应 `.md`，也没本地目录 → 完整走 recon + discovery，跑完直接写 `~/.bycli/sites/<site>/`，后面就有了。
