---
name: bycli-autofix
description: Automatically fix broken byCLI adapters when commands fail. Load this skill when an bycli command fails — it guides you through collecting a trace artifact, patching the adapter, retrying, and filing an upstream GitHub issue after a verified fix. Works with any AI agent.
allowed-tools: Bash(bycli:*), Bash(gh:*), Read, Edit, Write
---

# byCLI AutoFix — Automatic Adapter Self-Repair

When an `bycli` command fails because a website changed its DOM, API, or response schema, **automatically diagnose, fix the adapter, and retry** — don't just report the error.
...
- **`BROWSER_CONNECT`** (exit code 69) — **STOP.** Do not modify code. Tell the user to run `bycli doctor`.
- **CAPTCHA / rate limiting** — **STOP.** Not an adapter issue.

**Scope constraint:**
- **Only modify the file at `adapterSourcePath` in the trace `summary.md` front matter** — this is the authoritative adapter location (may be `clis/<site>/` in repo or `~/.bycli/clis/<site>/` for npm installs)
...
```bash
bycli doctor    # Verify extension + daemon connectivity
```

## When to Use This Skill

Use when `bycli <site> <command>` fails with repairable errors:
...
- **Retry with an alternative query or entry point.** If `bycli xiaohongshu search "X"` returns 0 but `bycli xiaohongshu search "X 攻略"` returns 20, the adapter is fine — the platform was shaping results for the first query.
- **Spot-check in a normal Chrome tab.** If the data is visible in the user's own browser but the adapter comes back empty, the issue is usually authentication state, rate limiting, or a soft block — not a code bug. The fix is `bycli doctor` / re-login, not editing source.
...
trace:
  ...
  bycliVersion: "..."
  traceId: "..."
  dir: "/path/to/.bycli/profiles/default/traces/..."
  summaryPath: "/path/to/.bycli/profiles/default/traces/.../summary.md"
  receiptPath: "/path/to/.bycli/profiles/default/traces/.../receipt.json"
```
...
Use `bycli browser` to inspect the live website. **Never use the broken adapter** — it will just fail again.
...
Read the adapter source file at `adapterSourcePath` from the trace summary front matter and make targeted fixes. This path is authoritative — it may be in the repo (`clis/`) or user-local (`~/.bycli/clis/`).
...
If the retry **passes**, the local adapter has drifted from upstream. File a GitHub issue so the fix flows back to `sovovs/byCLI`.
...
```markdown
## Summary
byCLI autofix repaired this adapter locally, and the retry passed.

## Adapter
- Site: `<site>`
- Command: `<command>`
- byCLI version: `<version from bycli --version>`
...
_Issue filed by byCLI autofix after a verified local repair._
```
...
- **Major redesign** — needs full adapter rewrite via `bycli-adapter-author` skill
...
```
1. User runs: bycli zhihu hot
   → Fails: SELECTOR "Could not find element: .HotList-item"

2. AI runs: bycli zhihu hot --trace retain-on-failure 2>trace-error.yaml
...
4. AI explores: bycli browser open https://www.zhihu.com/hot && bycli browser state
...
6. AI verifies: bycli zhihu hot
   → Success: returns hot topics

7. AI prepares upstream issue draft, shows it to the user

8. User approves → AI runs: gh issue create --repo sovovs/byCLI --title "[autofix] zhihu/hot: SELECTOR" --body "..."
```
