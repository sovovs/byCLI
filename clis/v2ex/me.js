...
cli({
    ...
    func: async (page) => {
        ...
        if (process.env.BYCLI_VERBOSE) {
            console.error('[bycli:v2ex] Navigating to /');
        }
    ...
    `);
        if (data.error) {
            if (process.env.BYCLI_VERBOSE) {
                console.error(`[bycli:v2ex:debug] Page Title: ${data.debug_title}`);
                console.error(`[bycli:v2ex:debug] Page Body: ${data.debug_body}`);
            }
