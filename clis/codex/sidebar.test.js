...
function fixtureDocument() {
    return el('document', {}, [
        ...
        project('bycli', '/Users/youngcan/bycli', [
            thread({ threadId: 'local:bycli-groups' }, '统一 bycli 二级命令分组', '1 天'),
        ]),
    ]);
}
...
describe('codex sidebar helpers', () => {
    ...
    it('flattens project rows with project filters', () => {
        const projects = collectCodexProjectsFromDocument(fixtureDocument());
        const rows = flattenCodexProjects(projects, { project: 'bycli' });

        expect(rows).toEqual([
            expect.objectContaining({
                Project: 'bycli',
                Index: 1,
                Title: '统一 bycli 二级命令分组',
                Updated: '1 天',
            }),
        ]);
    });
    ...
    it('does not match nested project paths when filtering by a parent label', () => {
        ...
        projects.push({
            index: 3,
            project: 'nested',
            projectPath: '/Users/youngcan/bycli/nested',
            collapsed: false,
            conversations: [
        ...
        expect(rows.map(row => row.Project)).toEqual(['bycli']);
    });
    ...
    it('selects a conversation by index within a project', () => {
        ...
        expect(result).toMatchObject({
            ok: true,
            project: 'bycli',
            conversation: '统一 bycli 二级命令分组',
            threadId: 'local:bycli-groups',
        });
    });
