 ...
 *
 * Usage:
 *   bycli web read --url "https://www.anthropic.com/research/..." --output ./articles
 *   bycli web read --url "https://..." --download-images false
 */
      ...
      (() => {
        ...
        const collectEmptyContainers = (root, scope, baseUrl) => {
          ...
          root.querySelectorAll?.(likely).forEach((el) => {
            if (scope === 'main' && el.closest?.('[data-bycli-iframe-source]')) return;
        ...
        const buildFrameSection = (frameBody, desc, fallbackLabel) => {
          ...
          section.setAttribute('data-bycli-iframe-source', desc.src);
...
const command = cli({
    ...
    func: async (page, kwargs, debug = false) => {
        ...
        const shouldDiagnose = boolish(kwargs.diagnose) || debug || !!process.env.BYCLI_VERBOSE;
