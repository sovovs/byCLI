...
describe('weixin search adapter', () => {
  it('is included in the compiled manifest as a public Sogou article search command', () => {
    ...
    const command = manifest.find(entry => entry.site === 'weixin' && entry.name === 'search');
  ...
  it('builds the Sogou article search URL and returns ranked rows', async () => {
    ...
    const page = makePage({
      ...
      invalidCount: 0,
      rows: [
        { title: 'First result', url: 'https://mp.weixin.qq.com/s/first', summary: 'Summary', publish_time: '1小时前' },
    ...
    expect(rows).toEqual([
      { rank: 11, page: 2, title: 'First result', url: 'https://mp.weixin.qq.com/s/first', summary: 'Summary', publish_time: '1小时前' },
    ]);
  });
