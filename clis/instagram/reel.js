...
import { Page as BrowserPage } from '@sovovs/bycli/browser/page';
...
function buildSafeTempVideoPath(filePath) {
    ...
    return path.join(os.tmpdir(), `bycli-instagram-video-real${ext}`);
}
    ...
    (() => {
      ...
      for (const root of roots) {
        ...
        for (const input of inputs) {
          ...
          input.setAttribute('data-bycli-reel-upload-index', String(index));
          selectors.push('[data-bycli-reel-upload-index="' + index + '"]');
          index += 1;
        }
      }
