...
async function gotoInstagramHome(page, forceReload = false) {
    if (forceReload) {
        await page.goto(`${INSTAGRAM_HOME_URL}?__bycli_reset=${Date.now()}`);
...
async function findUploadSelectors(page, mediaItems) {
    ...
    ((includesVideo) => {
      ...
      document.querySelectorAll('[data-bycli-ig-upload-index]').forEach((el) => el.removeAttribute('data-bycli-ig-upload-index'));
      const selectors = ordered.map((input, index) => {
        input.setAttribute('data-bycli-ig-upload-index', String(index));
        return '[data-bycli-ig-upload-index="' + index + '"]';
      });
...
function extractSelectorIndex(selector) {
    const match = selector.match(/data-bycli-ig-upload-index="(\d+)"/);
    if (!match)
        return null;
...
async function injectImageViaBrowser(page, imagePaths, selector) {
    ...
    const chunkKey = `__bycliInstagramUpload_${Date.now()}_${Math.random().toString(36).slice(2)}`;
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        const protocolCaptureEnabled = process.env.BYCLI_INSTAGRAM_CAPTURE === '1';
