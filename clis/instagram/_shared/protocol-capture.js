...
const DEFAULT_CAPTURE_VAR = '__bycli_ig_protocol_capture';
const DEFAULT_CAPTURE_ERRORS_VAR = '__bycli_ig_protocol_capture_errors';
...
export function buildInstallInstagramProtocolCaptureJs(captureVar = DEFAULT_CAPTURE_VAR, captureErrorsVar = DEFAULT_CAPTURE_ERRORS_VAR) {
    ...
    (() => {
      ...
      XMLHttpRequest.prototype.open = function(method, url) {
        this.__bycli_method = method;
        this.__bycli_url = url;
        this.__bycli_headers = {};
        return origOpen.apply(this, arguments);
      };
      XMLHttpRequest.prototype.setRequestHeader = function(name, value) {
        try {
          this.__bycli_headers = this.__bycli_headers || {};
          this.__bycli_headers[String(name)] = String(value);
        } catch {}
        return origSetRequestHeader.apply(this, arguments);
      };
      XMLHttpRequest.prototype.send = function(body) {
        this.addEventListener('load', () => {
          if (!shouldCapture(this.__bycli_url)) return;
          try {
            window[CAPTURE_VAR].push({
              kind: 'xhr',
              url: String(this.__bycli_url || ''),
              method: String(this.__bycli_method || 'GET').toUpperCase(),
              requestHeaders: this.__bycli_headers || {},
...
export async function dumpInstagramProtocolCaptureIfEnabled(page) {
    if (process.env.BYCLI_INSTAGRAM_CAPTURE !== '1')
        return;
