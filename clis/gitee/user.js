...
cli({
    ...
    func: async (page, args) => {
        ...
        if (!username) {
            throw new CliError('INVALID_ARGUMENT', 'Username is required', 'Use: bycli gitee user <username>');
        }
    ...
    `);
        ...
        if (domSnapshot.notFound) {
            throw new CliError('NOT_FOUND', `Gitee user "${username}" does not exist`, 'Check the username and retry: bycli gitee user <username>');
        }
