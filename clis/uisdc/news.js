...
function normalizeLimit(value) {
    ...
    if (!Number.isInteger(limit) || limit <= 0) {
        throw new ArgumentError('limit must be a positive integer', `Example: bycli uisdc news --limit ${DEFAULT_LIMIT}`);
    }
    if (limit > MAX_LIMIT) {
        throw new ArgumentError(`limit must be <= ${MAX_LIMIT}`, `Example: bycli uisdc news --limit ${MAX_LIMIT}`);
    }
    return limit;
}
