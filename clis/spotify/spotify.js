...
// or place them in ~/.bycli/spotify.env:
//   SPOTIFY_CLIENT_ID=your_id
//   SPOTIFY_CLIENT_SECRET=your_secret
const ENV_FILE = join(homedir(), '.bycli', 'spotify.env');
function loadEnv() {
...
// ── Token storage ─────────────────────────────────────────────────────────────
const TOKEN_FILE = join(homedir(), '.bycli', 'spotify-tokens.json');
function loadTokens() {
    try {
...
function saveTokens(tokens) {
    mkdirSync(join(homedir(), '.bycli'), { recursive: true });
...
async function getToken() {
    ...
    if (!tokens)
        throw new CliError('AUTH_REQUIRED', 'Not authenticated. Run: bycli spotify auth');
    if (!tokens.access_token || !tokens.refresh_token || !(tokens.expires_at > 0)) {
        throw new CliError('AUTH_CORRUPTED', 'Token file is corrupted. Run: bycli spotify auth');
    }
