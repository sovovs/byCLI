...
import { CDPBridge } from '@sovovs/bycli/browser/cdp';
import { resolveElectronEndpoint } from '@sovovs/bycli/launcher';
import { EXIT_CODES, getErrorMessage } from '@sovovs/bycli/errors';
// ─── Helpers ─────────────────────────────────────────────────────────
...
export async function startServe(opts = {}) {
    ...
    const envTimeoutSeconds = parseEnvTimeout('BYCLI_ANTIGRAVITY_TIMEOUT', 120);
    ...
    async function ensureConnected() {
        ...
        // If multiple editor windows are open, set BYCLI_CDP_TARGET to the window title.
        if (process.env.BYCLI_CDP_TARGET) {
            console.error(`[serve] Using BYCLI_CDP_TARGET=${process.env.BYCLI_CDP_TARGET}`);
        }
        ...
        console.error(`[serve] Connecting via CDP (target pattern: "${process.env.BYCLI_CDP_TARGET}")...`);
        cdp = new CDPBridge();
        try {
    ...
    `);
        if (!hasUI) {
            console.error('[serve] ⚠️  Warning: chat UI elements not found in this target. Try setting BYCLI_CDP_TARGET to the correct window title.');
        }
        return page;
    }
