 ...
 */
import { ArgumentError, CliError, EmptyResultError } from '@sovovs/bycli/errors';
...
async function ensureDoubanReady(page) {
  ...
  `);
    if (state?.blocked) {
        throw new CliError('AUTH_REQUIRED', 'Douban requires a logged-in browser session before these commands can load data.', 'Please sign in to douban.com in the browser that bycli reuses, then rerun the command.');
    }
}
