import { ArgumentError, AuthRequiredError, CliError, CommandExecutionError } from '@sovovs/bycli/errors';
...
function ensureNotebookUuid(candidate) {
    if (!NOTEBOOK_UUID_RE.test(candidate)) {
        throw new CliError('NOTEBOOKLM_INVALID_NOTEBOOK', `NotebookLM notebook id "${candidate}" is not a valid UUID`, 'Pass a notebook id from `bycli notebooklm list` or a full notebook URL like https://notebooklm.google.com/notebook/<uuid>.');
    }
    return candidate;
}
export function parseNotebooklmNotebookTarget(value) {
    ...
    if (!normalized) {
        throw new CliError('NOTEBOOKLM_INVALID_NOTEBOOK', 'NotebookLM notebook id is required', 'Pass a notebook id from `bycli notebooklm list` or a full notebook URL.');
    }
    if (/^https?:\/\//i.test(normalized)) {
        ...
        if (parsed.protocol !== 'https:' || parsed.hostname !== NOTEBOOKLM_DOMAIN || parsed.username || parsed.password || parsed.port) {
            throw new CliError('NOTEBOOKLM_INVALID_NOTEBOOK', 'NotebookLM notebook URL must be a canonical https://notebooklm.google.com URL', 'Pass a notebook id from `bycli notebooklm list` or a full NotebookLM notebook URL.');
        }
...
export function getNotebooklmAuthuser() {
    const v = process.env.BYCLI_NOTEBOOKLM_AUTHUSER;
