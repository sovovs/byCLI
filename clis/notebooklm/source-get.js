...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (state.kind !== 'notebook') {
            throw new EmptyResultError('bycli notebooklm source-get', 'No NotebookLM notebook is open in the adapter session. Run `bycli notebooklm open <notebook>` first.');
        }
        ...
        if (rows.length === 0) {
            throw new EmptyResultError('bycli notebooklm source-get', 'No NotebookLM sources were found on the current page.');
        }
        ...
        if (matched)
            return [matched];
        throw new EmptyResultError('bycli notebooklm source-get', `Source "${query}" was not found in the current notebook.`);
    },
});
