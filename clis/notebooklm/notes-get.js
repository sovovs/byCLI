...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (state.kind !== 'notebook') {
            throw new EmptyResultError('bycli notebooklm notes-get', 'No NotebookLM notebook is open in the adapter session. Run `bycli notebooklm open <notebook>` first.');
        }
        ...
        if (listed) {
            throw new EmptyResultError('bycli notebooklm notes-get', `Note "${query}" is listed in Studio, but bycli currently reads note content only from the visible note editor. Open that note in NotebookLM, then retry.`);
        }
        throw new EmptyResultError('bycli notebooklm notes-get', `Note "${query}" was not found in the current notebook.`);
    },
});
