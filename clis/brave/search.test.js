...
describe('brave search', () => {
  ...
  it('rejects empty query, invalid limit, and invalid offset before navigation', async () => {
    ...
    await expect(command.func(page, { keyword: 'bycli', limit: 19 })).rejects.toMatchObject({ code: 'ARGUMENT' });
    await expect(command.func(page, { keyword: 'bycli', limit: 5, offset: -1 })).rejects.toMatchObject({ code: 'ARGUMENT' });
  ...
  it('unwraps browser envelopes and returns ranked HTTPS rows', async () => {
    const page = createPageMock({
      session: 'site:brave',
      data: [['byCLI', 'https://github.com/sovovs/byCLI', 'CLI browser tooling']],
    });

    await expect(command.func(page, { keyword: 'bycli', limit: 1, offset: 1 })).resolves.toEqual([{
      rank: 19,
      title: 'byCLI',
  ...
  it('fails typed instead of silently returning [] for malformed extraction payloads', async () => {
    ...
    await expect(command.func(page, { keyword: 'bycli', limit: 1 })).rejects.toMatchObject({
      code: 'COMMAND_EXEC',
