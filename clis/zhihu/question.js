...
import { AuthRequiredError, CliError } from '@sovovs/bycli/errors';
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (!/^\d+$/.test(questionId)) {
            throw new CliError('INVALID_INPUT', 'Question ID must be numeric', 'Example: bycli zhihu question 123456789');
        }
        ...
        if (sort !== 'default' && sort !== 'created') {
            throw new CliError('INVALID_INPUT', 'Sort must be one of: default, created', 'Example: bycli zhihu question 123456789 --sort created');
        }
