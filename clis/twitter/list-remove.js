...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        const uiResult = await page.evaluate(`(async () => {
            ...
            try {
                if (!window.__bycliListMutations) {
                    window.__bycliListMutations = [];
                    ...
                    window.fetch = async function(...args) {
                        ...
                        try { resp = await origFetch(...args); } catch (err) {
                            if (/ListAddMember|ListRemoveMember|lists\\/members\\/(create|destroy)/.test(url)) {
                                window.__bycliListMutations.push({ url, method, status: 0, error: String(err), ts: Date.now() });
                            }
                            throw err;
                        }
                        if (/ListAddMember|ListRemoveMember|lists\\/members\\/(create|destroy)/.test(url)) {
                            window.__bycliListMutations.push({ url, method, status: resp.status, ts: Date.now() });
                        }
                        return resp;
                    };
                }
                window.__bycliListMutations.length = 0;
                ...
                return {
                    ...
                    mutationsBefore: window.__bycliListMutations.length,
                };
            } catch (e) {
