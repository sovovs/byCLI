 ...
 *
 * Usage:
 *   bycli twitter download elonmusk --limit 50 --output ./twitter
 *   bycli twitter download --tweet-url https://x.com/xxx/status/123 --output ./twitter
 */
import { cli, Strategy } from '@sovovs/bycli/registry';
import { ArgumentError, AuthRequiredError, CliError, CommandExecutionError, EmptyResultError } from '@sovovs/bycli/errors';
import { formatCookieHeader } from '@sovovs/bycli/download';
import { downloadMedia } from '@sovovs/bycli/download/media-download';
import {
...
cli({
    ...
    func: async (page, kwargs) => {
        try {
            ...
            if (!username) {
                throw new ArgumentError('twitter download username must be a valid Twitter/X handle', 'Example: bycli twitter download @jack --limit 20');
            }
