import { ArgumentError, AuthRequiredError, selectorError, EmptyResultError, CommandExecutionError } from '@sovovs/bycli/errors';
 ...
 * `bycli twitter profile <user>` to read a per-user follower count.
 */
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (rawUser && !targetUser) {
            throw new ArgumentError('twitter followers user must be a valid Twitter/X handle', 'Example: bycli twitter followers @elonmusk --limit 100');
        }
        if (!targetUser) {
        ...
        if (!targetUser) {
            throw new ArgumentError('twitter followers user cannot be empty', 'Example: bycli twitter followers @elonmusk --limit 100');
        }
