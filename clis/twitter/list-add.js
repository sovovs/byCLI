...
cli({
    ...
    browser: true,
    args: [
        { name: 'listId', positional: true, type: 'string', required: true, help: 'Numeric ID of the list you own (e.g. from `bycli twitter lists`)' },
    ...
    func: async (page, kwargs) => {
        ...
        if (!listId || !/^\d+$/.test(listId)) {
            throw new ArgumentError(`Invalid listId: ${JSON.stringify(kwargs.listId)}. Expected numeric ID.`, 'Example: bycli twitter list-add 123456789 alice');
        }
        if (!username) {
            throw new ArgumentError('twitter list-add username is required', 'Example: bycli twitter list-add 123456789 alice');
        }
        ...
        // bycli >=1.7.x wraps page.evaluate return values as { session, data }.
        ...
        // Don't unwrap listsData: bycli spreads GraphQL response to top-level + adds session;
