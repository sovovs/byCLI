    ...
    func: async (args) => {
        ...
        try {
            resp = await fetch(url, {
                headers: {
                    'User-Agent': 'bycli/1.0 (+https://github.com/sovovs/byCLI)',
        ...
        if (!page || page.missing) {
            throw new EmptyResultError('wikipedia page', `No article "${title}" on ${lang}.wikipedia.org. Try \`bycli wikipedia search\` first.`);
        }
