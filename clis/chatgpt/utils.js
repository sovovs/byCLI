 ...
 * Cross-platform: works on Linux/macOS/Windows via byCLI's CDP browser automation.
 */

import { htmlToMarkdown } from '@sovovs/bycli/utils';
import { ArgumentError, AuthRequiredError, CommandExecutionError, TimeoutError } from '@sovovs/bycli/errors';
...
function buildComposerLocatorScript() {
    const markerAttr = 'data-bycli-chatgpt-composer';
    return `
      const isVisible = (el) => {
...
export function requireNonEmptyPrompt(prompt, commandName) {
    ...
    if (!text) {
        throw new ArgumentError(
            ...
            `Example: bycli ${commandName} "hello"`,
        );
    }
    return text;
}
...
export function parseChatGPTConversationId(value) {
    ...
    throw new ArgumentError(
        ...
        'Example: bycli chatgpt detail 123e4567-e89b-12d3-a456-426614174000',
    );
}
