...
async function prepareCustomCoverInput(page) {
    ...
    for (let attempt = 0; attempt < COVER_INPUT_WAIT_ATTEMPTS; attempt += 1) {
      ...
      document
        .querySelectorAll('[data-bycli-cover-input="1"]')
        .forEach((el) => el.removeAttribute('data-bycli-cover-input'));
      target.setAttribute('data-bycli-cover-input', '1');
      return { ok: true, selector: '[data-bycli-cover-input="1"]' };
    }`));
