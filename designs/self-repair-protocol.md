...
When an AI agent uses `bycli <site> <command>` and the command fails because the site changed DOM, API, or response schema, the agent should automatically repair the adapter and retry without human intervention or pre-written spec files.
...
```text
Agent runs: bycli <site> <command> [args...]
...
- `~/.bycli/clis/<site>/*.js` — user-local adapters in npm install scenarios
...
| Browser bridge not connected | Extension/daemon not running | Tell user to run `bycli doctor` |
...
| AutoFix skill protocol | Done | `skills/bycli-autofix/SKILL.md` |

### Delivery Mechanism

The `bycli-autofix` skill is the portable self-repair protocol. Any AI agent can load this skill to get the workflow.

No separate diagnostic env var is required. The runtime has two control axes:

```text
-v / BYCLI_VERBOSE              human-readable logs
...
The `bycli-autofix` skill instructs agents:

1. When `bycli <site> <command>` fails, don't just report the error.
...
6. Retry the original command.
7. If the retry passes, ask whether to file an upstream GitHub issue for `sovovs/byCLI`.
...
- Phase 1: self-repair via `bycli-autofix` skill and trace artifacts.
...
No new commands. No new scripts. The agent loads the `bycli-autofix` skill and uses bycli normally:

```bash
# Agent runs a command as part of its task
bycli weibo hot --limit 5 -f json

# If it fails, the agent automatically:
# 1. Runs bycli weibo hot --limit 5 -f json --trace retain-on-failure 2>trace-error.yaml
...
# 5. Retries: bycli weibo hot --limit 5 -f json
# 6. If retry passes, asks whether to file an upstream issue
# 7. If approved, runs `gh issue create --repo sovovs/byCLI ...`
